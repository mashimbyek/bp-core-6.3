...
public class DefaultWorkflowTemplateActivationServiceTest
{
	...
	public void setUp()
	{
		availableLocales = new Locale[] { Locale.ENGLISH, Locale.GERMAN, Locale.ITALY, Locale.CHINA };
		MockitoAnnotations.initMocks(this);
		service.setWorkflowActivationSupportedTypes(Sets.newSet(TEST_ITEM_TYPE));
		when(typeService.isAssignableFrom(TEST_ITEM_TYPE, TEST_ITEM_TYPE)).thenReturn(true);
		doAnswer(mock -> ((Locale) mock.getArguments()[0]).toLanguageTag()).when(localizationService).getDataLanguageIsoCode(
				any(Locale.class));
	}
	...
	public void testCreateWorkflowTemplateActivationCtxForSave() throws TypeNotFoundException
	{
		...
		when(itemModelContext.isNew()).thenReturn(false);
	...
	public void testCollectCurrentValues()
	{
		...
		assertThat(((Map<String, Object>) originalValues.get("c")).get(Locale.CANADA.toLanguageTag())).isEqualTo(
				getCurrentValue("c", Locale.CANADA));
		assertThat(((Map<String, Object>) originalValues.get("c")).get(Locale.CHINA.toLanguageTag())).isEqualTo(
				getCurrentValue("c", Locale.CHINA));

		assertThat(originalValues.get("d")).isInstanceOf(Map.class);
		assertThat(((Map<String, Object>) originalValues.get("d")).get(Locale.ENGLISH.toLanguageTag())).isEqualTo(
				getCurrentValue("d", Locale.ENGLISH));
		assertThat(((Map<String, Object>) originalValues.get("d")).get(Locale.GERMAN.toLanguageTag())).isEqualTo(
				getCurrentValue("d", Locale.GERMAN));
	...
	public void testCollectOriginalValues()
	{
		...
		assertThat(originalValues.get("c")).isInstanceOf(Map.class);
		assertThat(((Map<String, Object>) originalValues.get("c")).get(Locale.CANADA.toLanguageTag())).isEqualTo(
				getOriginalValue("c", Locale.CANADA));
		assertThat(((Map<String, Object>) originalValues.get("c")).get(Locale.CHINA.toLanguageTag())).isEqualTo(
				getOriginalValue("c", Locale.CHINA));

		assertThat(originalValues.get("d")).isInstanceOf(Map.class);
		assertThat(((Map<String, Object>) originalValues.get("d")).get(Locale.ENGLISH.toLanguageTag())).isEqualTo(
				getOriginalValue("d", Locale.ENGLISH));
		assertThat(((Map<String, Object>) originalValues.get("d")).get(Locale.GERMAN.toLanguageTag())).isEqualTo(
				getOriginalValue("d", Locale.GERMAN));
	...
	{
		final BiFunction<String, Locale, Object> localizedValueSupp = (attribute, locale) -> locale.toLanguageTag().concat(
				attribute);
		...
		assertThat(((Map) localizedValues.get("a")).get(Locale.CANADA.toLanguageTag())).isEqualTo(
				Locale.CANADA.toLanguageTag().concat("a"));
		assertThat(((Map) localizedValues.get("a")).get(Locale.CHINA.toLanguageTag())).isEqualTo(
				Locale.CHINA.toLanguageTag().concat("a"));


		assertThat(localizedValues.get("b")).isInstanceOf(Map.class);
		assertThat(((Map) localizedValues.get("b"))).hasSize(3);
		assertThat(((Map) localizedValues.get("b")).get(Locale.ENGLISH.toLanguageTag())).isEqualTo(
				Locale.ENGLISH.toLanguageTag().concat("b"));
		assertThat(((Map) localizedValues.get("b")).get(Locale.GERMAN.toLanguageTag())).isEqualTo(
				Locale.GERMAN.toLanguageTag().concat("b"));
		assertThat(((Map) localizedValues.get("b")).get(Locale.ITALY.toLanguageTag())).isEqualTo(
				Locale.ITALY.toLanguageTag().concat("b"));
	}
	...
	public void testCollectModifiedAttributes()
	{
		...
		final ItemModelContext itemModelContext = mockItemModelCtxWithAttributes(Sets.newSet("b", "c", "d"), Collections.emptySet());
	...
	{
		final DataType dataType = mockDataTypeWithAttributes("a", "b", "c");
		final ItemModelContext itemModelContext = mockItemModelCtxWithAttributes(Collections.emptySet(), Sets.newSet("b", "c", "d"));
	...
	{
		when(typeService.isAssignableFrom("Product", "VariantProduct")).thenReturn(true);
		when(typeService.isAssignableFrom("Product", "Catalog")).thenReturn(false);
	...
	protected ItemModelContext mockItemModelCtxWithAttributes(final Set<String> attributes, final Set<String> localizedAttributes)
	{
		...
		when(itemModelContext.getDirtyLocalizedAttributes()).thenReturn(
				Arrays.stream(availableLocales).collect(Collectors.toMap(loc -> loc, loc -> localizedAttributes)));
		doAnswer(mock -> getOriginalValue((String) mock.getArguments()[0])).when(itemModelContext).getOriginalValue(
				any(String.class));
		doAnswer(mock -> {
