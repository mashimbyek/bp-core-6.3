...
public class DefaultObjectFacadeTest extends ServicelayerTransactionalTest
{
	...
	public void before() throws TypeNotFoundException {
		...
		typeFacade = mock(TypeFacade.class);
		...
		defaultObjectFacadeStrategy.setTypeFacade(typeFacade);
		defaultObjectFacadeStrategy.setWorkflowTemplateActivationService(workflowTemplateActivationService);
		...
		itemModificationHistoryService.setTypeFacade(typeFacade);

		dataAttribute = mock(DataAttribute.class);
		dataType = mock(DataType.class);
		when(typeFacade.getAttribute(any(Object.class), any(String.class))).thenReturn(dataAttribute);
		when(typeFacade.load(any(String.class))).thenReturn(dataType);
		...
		customStrategy.setWorkflowTemplateActivationService(workflowTemplateActivationService);
		...
		objectFacade.setTypeFacade(typeFacade);
		objectFacade.setEventQueue(eventQueue);
		objectFacade.setCockpitProperties(cockpitProperties);
	...
	@Test
	public void testPersistWithContextShouldThrowException()
	{
		TestUtils.disableFileAnalyzer(FILE_ANALYZER_REASON);
		try
		{
	...
	@Test
	public void testSaveObjectWithAccessToPrivateAttributes()
	{
		// given
		when(typeFacade.getAttribute(any(Object.class), any(String.class))).thenAnswer(new Answer<Object>()
		{
	...
	private void executeIsNewTest()
	{
		try
		{
			UserModel user = objectFacade.create("User");
			assertThat(objectFacade.isNew(user)).as("Object hasn't been persisted but isn't recognized as new").isTrue();
			user.setUid("temp1");
			user = objectFacade.save(user);
			assertThat(objectFacade.isNew(user)).as("Object has been saved but is recognized as new").isFalse();
		}
		catch (ObjectCreationException | ObjectSavingException e)
		{
			Assert.fail("Unexpected exception occurred:" + e.getMessage());
	...
	{
		Mockito.when(cockpitProperties.getProperty(DefaultObjectFacade.COCKPITNG_CRUD_COCKPIT_EVENT_NOTIFICATION)).thenReturn("true");

		final ProductModel product1 = new ProductModel();
		final ProductModel product2 = new ProductModel();

		final List<ProductModel> objectsToDelete = Arrays.asList(product1, product2);

		objectFacade.delete(objectsToDelete, new DefaultContext());

		final org.hamcrest.Matcher<DefaultCockpitEvent> publishedObjectsMatch = new org.hamcrest.BaseMatcher<DefaultCockpitEvent>(){
			@Override
			...
			public boolean matches(final Object o) {
				return CollectionUtils.isEqualCollection((Collection)((DefaultCockpitEvent)o).getData(), objectsToDelete);
			}
		};

		Mockito.verify(eventQueue).publishEvent(Mockito.argThat(publishedObjectsMatch));
	}
}
