 ...
 *
 * @deprecated The AcceleratorCMSBodyTag is deprecated. Load the live edit CSS and JavaScript files you want as required
 *             rather than relying on the default behaviour of he CMSBodyTag.
 */
@Deprecated
public class AcceleratorCMSBodyTag extends CMSBodyTag
{
	...
	protected String getBodyTagCssClass(final AbstractPageModel currentPage) // NOSONAR
	{
	...
	protected String getBodyTagStyle(final AbstractPageModel currentPage) // NOSONAR
	{
		if (getCssClass() != null && !getCssClass().isEmpty())
		{
	...
	protected String getBodyOnLoad(final AbstractPageModel currentPage)
	{
		if (isPreviewDataModelValid())
		{
			...
			return " onload=\"getCurrentPageLocation(window.location.href, '" + currentPagePk + "' , currentUserId "
					+ ", currentJaloSessionId)\" ";
		}
		return "";
	}

	protected String getBodyOnClick(final AbstractPageModel currentPage) // NOSONAR
	{
		if (isLiveEdit())
		{
	...
	protected String getLiveEditCssAndJs(final AbstractPageModel currentPage) // NOSONAR
	{
		// Load the CSS
		// Load the JS async
		return "<link rel=\"stylesheet\" type=\"text/css\" href=\"" + getLiveEditCssPath() + "\"/>\n"
	...
	protected String getScriptInBodyTag(final AbstractPageModel currentPage) // NOSONAR
	{
		if (isPreviewDataModelValid())
		{
