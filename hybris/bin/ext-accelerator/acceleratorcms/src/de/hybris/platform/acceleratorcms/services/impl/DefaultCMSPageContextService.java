...
/**
 * Default implementation of {@link CMSPageContextService}
 */
public class DefaultCMSPageContextService implements CMSPageContextService
{
	...
	private static final String PARAMETER_REQUEST_NOT_NULL_MSG = "Parameter request must not be null";
	...
	{
		validateParameterNotNull(request, PARAMETER_REQUEST_NOT_NULL_MSG);
			...
			final RestrictionData restrictionData)
	{
		...
		if (page != null)
		{
			...
			{
				LOG.debug("Page " + page.getName() + " was set for cms page request context data.");
			}
		}
