		...
		return getImpersonationService().executeInContext(impersonationContext,
				new ImpersonationService.Executor<List<PK>, Throwable>()
				{
					...
					public List<PK> execute() throws Exception
					{
						...
						LOG.debug("Running query: " + flexibleSearchQuery.getQuery());
						flexibleSearchQuery.setResultClassList(Collections.singletonList(PK.class));
						flexibleSearchQuery.addQueryParameters(prepareParameters(message, getParameters()));

						final SearchResult<PK> searchResult = getFlexibleSearchService().search(flexibleSearchQuery);
