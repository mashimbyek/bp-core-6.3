...
{
	protected static final String QUERY = "SELECT {" + EmailPageModel.PK + "} " + "FROM {" + EmailPageModel._TYPECODE + " AS p " + "JOIN "
			+ EmailPageTemplateModel._TYPECODE + " AS t ON {p:" + EmailPageModel.MASTERTEMPLATE + "}={t:" + EmailPageTemplateModel.PK
			+ "}} " + "WHERE {t:" + EmailPageTemplateModel.UID + "}=?templateUid AND {t:" + EmailPageTemplateModel.CATALOGVERSION
			+ "}=?catalogVersion";
