...
public class ProductConverter extends AbstractConverter<Message<ProductModel>, Product>
{
	 ...
	 */
	public interface Condition // NOSONAR
	{
		String NEW = "New";
		String USED = "Used";
	...
	protected String getProductCondition(final ProductModel productModel) // NOSONAR
	{
		//always return new
	...
	protected String generateGoogleProductCategory(final ProductModel productModel) // NOSONAR
	{
	...
	public void populate(final Message<ProductModel> message, final Product product)
	{
		...
		if (productModel != null)
		{
			...
			product.setPrice(
					info != null ? (info.getPriceValue() != null ? Double.toString(info.getPriceValue().getValue()) : "") : "");
			...
			if (message.getHeaders().get("event") instanceof ExportDataEvent)
			{
				...
				if (ede.getSite() != null)
				{
					...
					product.setImageLink(productModel.getPicture() != null
							? mediaUrl != null ? mediaUrl + productModel.getPicture().getURL() : productModel.getPicture().getURL()
							: "");
