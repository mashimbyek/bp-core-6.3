...
public class CartRemovalJobTest extends ServicelayerTransactionalTest
{
	...
	public void testSavedCartsWithExceededCartMaxAge() throws InterruptedException
	{
		...
		// as we have added hooks for after save in accelerator, we remove the cart from the session & request a new session cart.
		// by doing so, in this test while performs cartRemovalCronJob we are removing the new session cart object from the database,
		// which results into an error when HybrisJUnit4Test.finish is called, which calls setUser which checks for cart &
		// the ref. is still there in the JaloSession and then it tries to fetch it from cache while it performs remote.refresh &
		// doesnt find the item in the DB, throws an exception.
