...
public class HostedOrderPageMockController
{
	...
	private static final Logger LOG = Logger.getLogger(HostedOrderPageMockController.class);
	...
	/**
	 * Creation of Map from paramString. Loop step is 2. So we take key from 0 and value from 1 then key from 2 and value
	 * from 3.
	 * 
	 */
	protected Map<String, String> deserializeParameterMap(final String paramString)
	{
		...
		if (StringUtils.isNotBlank(paramString))
		{
			...
			{
				results.put(params[i], i + 1 >= params.length ? "" : params[i + 1]);
			}
		}
		return results;
	}
