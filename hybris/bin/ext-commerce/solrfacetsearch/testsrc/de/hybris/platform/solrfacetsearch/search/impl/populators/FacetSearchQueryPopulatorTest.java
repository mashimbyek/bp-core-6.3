...
public class FacetSearchQueryPopulatorTest
{
	...
	public void setUp()
	{
		...
		final DisMaxFreeTextQueryBuilder disMaxFreeTextQueryBuilder = new DisMaxFreeTextQueryBuilder();
	...
	public void testPopulateWithQueryBoosts()
	{
		...
		searchQueryConverterData.getSearchQuery().addBoost(FIELD1, SearchQuery.QueryOperator.EQUAL_TO, "test1", 10f,
				BoostField.BoostType.MULTIPLICATIVE);
		searchQueryConverterData.getSearchQuery().addBoost(FIELD2, SearchQuery.QueryOperator.EQUAL_TO, "test2", 11f,
				BoostField.BoostType.MULTIPLICATIVE);
		searchQueryConverterData.getSearchQuery().addBoost(FIELD1, SearchQuery.QueryOperator.EQUAL_TO, "test1", 12f,
				BoostField.BoostType.ADDITIVE);
		searchQueryConverterData.getSearchQuery().addBoost(FIELD2, SearchQuery.QueryOperator.EQUAL_TO, "test2", 13f,
				BoostField.BoostType.ADDITIVE);
		...
		//then
		final String expectedQuery = "{!boost b=\"product(map(query({!v='translatedField1\\:test'}),0,0,1,10.0),map(query({!v='translatedField2\\:test'}),0,0,1,11.0))\"} *:* AND ({!func v=\"sum(map(query({!v='translatedField1\\:test'}),0,0,0,12.0),map(query({!v='translatedField2\\:test'}),0,0,0,13.0))\"})";
		assertEquals(expectedQuery, solrQuery.getQuery());
	}
}
