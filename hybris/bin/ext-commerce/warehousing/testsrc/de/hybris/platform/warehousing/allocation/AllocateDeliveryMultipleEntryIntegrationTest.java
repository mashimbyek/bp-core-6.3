...
public class AllocateDeliveryMultipleEntryIntegrationTest extends BaseWarehousingIntegrationTest
{
	...
	public void shouldAllocate_From1Pos1Warehouse()
	{
		...
		do
		{
			...
			if (entry.getOrderEntry().getProduct().getCode().equals(Products.CODE_CAMERA))
			{
		...
		assertEquals(MONTREAL_CAMERA_QTY, ((OrderEntryModel) order.getEntries().get(0)).getQuantityAllocated());
		assertEquals(MONTREAL_MEMORY_CARD_QTY, ((OrderEntryModel) order.getEntries().get(1)).getQuantityAllocated());
	}
	...
	public void shouldAllocate_From2Pos2Warehouse()
	{
		...
		assertEquals(MONTREAL_CAMERA_QTY, ((OrderEntryModel) order.getEntries().get(0)).getQuantityAllocated());
		assertEquals(BOSTON_MEMORY_CARD_QTY, ((OrderEntryModel) order.getEntries().get(1)).getQuantityAllocated());
	}
