...
public class NoRestrictionsStrategyTest
{
	 ...
	 * It verifies the {@link PointOfServiceModel} where the order entries were sourced<br>
	...
	public void testSourcingNoRestriction1Location()
	{
		...
		final SourcingLocation location1 = SourcingLocationBuilder.aSourcingLocation()
				.withCustomAvailabilityProductQuantity(propaneBarbecue, Long.valueOf(STOCK_RECIFE_PROPANE_BARBECUE))
				.withCustomAvailabilityProductQuantity(charcoalBarbecue, Long.valueOf(STOCK_RECIFE_CHARCOAL_BARBECUE))
				.withWarehouseCode(Warehouse.RECIFE.name()).build();

		final SourcingContext context = SourcingContextBuilder.aSourcingContext()
				.withOrderEntry(createOrderEntry(Long.valueOf(QTY_PROPANE_BARBECUE), propaneBarbecue))
				.withOrderEntry(createOrderEntry(Long.valueOf(QTY_CHARCOAL_BARBECUE), charcoalBarbecue)).withSourcingLocation(location1).build();

		when(fitnessCalculationMock.sortByFitness(context)).thenReturn(
	...
	public void testSourcingNoRestrictionSplit2Location()
	{
		...
		final SourcingLocation location2 = SourcingLocationBuilder.aSourcingLocation()
				.withCustomAvailabilityProductQuantity(propaneBarbecue, Long.valueOf(STOCK_OLINDA_PROPANE_BARBECUE))
				.withWarehouseCode(Warehouse.OLINDA.name()).build();

		final SourcingContext context = SourcingContextBuilder.aSourcingContext()
				.withOrderEntry(createOrderEntry(Long.valueOf(QTY_PROPANE_BARBECUE), propaneBarbecue))
				.withOrderEntry(createOrderEntry(Long.valueOf(QTY_CHARCOAL_BARBECUE), charcoalBarbecue)).withSourcingLocation(location1)
	...
	public void testSourceCompleteFrom3Locations()
	{
		...
		final SourcingLocation location3 = SourcingLocationBuilder.aSourcingLocation()
				.withCustomAvailabilityProductQuantity(propaneBarbecue, Long.valueOf(STOCK_PAULISTA_PROPANE_BARBECUE))
...
.filter(result -> result.getWarehouse().getCode().equals(Warehouse.PAULISTA.name()))
	...
	public void testNothingSourced()
	{
		...
		final SourcingLocation location1 = SourcingLocationBuilder.aSourcingLocation()
				.withCustomAvailabilityProductQuantity(propaneBarbecue, Long.valueOf(STOCK)).withWarehouseCode(Warehouse.RECIFE.name()).build();

		final SourcingLocation location2 = SourcingLocationBuilder.aSourcingLocation()
				.withCustomAvailabilityProductQuantity(propaneBarbecue, Long.valueOf(STOCK)).withWarehouseCode(Warehouse.OLINDA.name()).build();
		...
		when(fitnessCalculationMock.sortByFitness(Mockito.anyCollectionOf(SourcingLocation.class))).thenReturn(
				new ArrayList<>(context.getSourcingLocations()));
		noRestrictionsStrategy.source(context);
	...
	public void testPartiallySourced()
	{
		...
		final SourcingLocation location2 = SourcingLocationBuilder.aSourcingLocation()
				.withCustomAvailabilityProductQuantity(propaneBarbecue, Long.valueOf(STOCK_OLINDA_CHARCOAL_BARBECUE))
				.withWarehouseCode(Warehouse.OLINDA.name()).build();
	...
	{
		final OrderEntryModel entry = spy(new OrderEntryModel());
		entry.setQuantity(quantity);
		entry.setProduct(product);
		when(entry.getQuantityUnallocated()).thenReturn(quantity);
		return entry;
	}
