...
import de.hybris.platform.warehousing.util.models.ContentCatalogs;
...
public class DataIntegrityTest extends BaseWarehousingIntegrationTest
{
	...
	private CountryModel countryFrance;
	...
	private ContentCatalogModel contentCatalogPrimary;
	...
	public void setUp()
	{
		...
		countryFrance = countries.France();
		...
		contentCatalogPrimary =contentCatalogs.contentCatalog_online();
		catalogVersionStaging = catalogVersions.Staging();
		catalogVersionOnline = catalogVersions.Online();
	...
	public void verifyCountryIntegrity()
	{
		...
		final CountryModel france = countryDao.findCountriesByCode(Countries.ISOCODE_FRANCE).iterator().next();
		assertEquals(countryFrance.getIsocode(), france.getIsocode());
	}
	...
	public void verifyBaseStoreIntegrity_WithDeliveryModes()
	{
		...
		assertEquals(contentCatalogPrimary, baseStore.getCatalogs().iterator().next());
		assertEquals(currencyUSD, baseStore.getDefaultCurrency());
		assertEquals(languageEnglish, baseStore.getDefaultLanguage());
		assertThat(baseStore.getDeliveryCountries(), containsInAnyOrder(countryCanada, countryUSA, countryFrance));
	...
	public void verifyProductIntegrity_NoPrice()
	{
		...
		assertEquals(catalogVersionOnline, product.getCatalogVersion());
	}
