...
public class ProductPercentageDiscountPromotionTest extends AbstractPromotionTest
{
	...
	private void testPromotionBody() throws CalculationException
	{
		...
		assertEquals("before updatePromotions(ProductPercentageDiscountPromotion)", FIRST_PRODUCT_NO_PROMO,
				cart.getTotalPrice().doubleValue(), 0.01);

		final PromotionGroupModel promotionGroup = defaultPromotionsService.getPromotionGroup("prGroup6");
		...
		assertEquals("both products without promotion after recalculate", BOTH_PRODUCTS_NO_PROMO,
				cart.getTotalPrice().doubleValue(), 0.01);
		...
		//modelService.refresh(cart);
		//309.13 + 86.80 = 395.93
		assertEquals("two products with promotion", FIRST_PRODUCT_APPLIED_PROMO_SECOND_NONE_PROMO,
				cart.getTotalPrice().doubleValue(), 0.01);
	}
