...
{
	private final static Logger LOG = LoggerFactory.getLogger(Order2PromotionResultRelationTest.class);
	...
	private void testActionsSize(final int promotionActionSize)
	{
		...
		if (isCacheActivated)
		{
			...
			if (promotionResults != null)
			{
				...
				{
					final Collection<AbstractPromotionAction> promotionActions = promotionResult.getActions();
					if (isNotEmpty(promotionActions))
					{
						actionsSize += promotionActions.size();
					}
				}
			}
		}
