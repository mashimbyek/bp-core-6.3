		...
		AbstractProductPopulator<SOURCE, TARGET>
{
	...
	public void populate(final SOURCE productModel, final TARGET productData) throws ConversionException
	{
		...
		if (hasBundleRules(productModel) && hasBundleTemplates(productData))
		{
	...
	protected boolean hasBundleRules(final ProductModel productModel)
	{
		return productModel != null && productModel.getTargetBundleRules() != null
				&& productModel.getConditionalBundleRules() != null;
	}

	protected boolean hasBundleTemplates(final ProductData productData)
	{
		return productData != null && productData.getBundleTemplates() != null;
	}
	 ...
	 * @return List of product code as string.
	 */
	protected Collection<String> extractProductCode(final Collection<ProductModel> productList)
	{
		final Collection<String> disableProductList = new ArrayList<>();
	...
	protected void disableProducts(final BundleTemplateData data, final Collection<String> disableProduct)
	{
		...
		{
			disableProduct.stream()
					.forEach(product -> data.getProducts().stream()
							.filter(productData -> StringUtils.equals(productData.getCode(), product))
							.findFirst()
							.ifPresent(productData -> productData.setDisabled(DISABLE_PRODUCT)));
		}
	}
