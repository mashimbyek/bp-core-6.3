		...
		AbstractBundleOrderPopulator<S, T>
{
	...
	protected List<OrderEntryData> getPotentialProducts(final AbstractOrderModel source, final AbstractOrderData target)
	{
		...
		{
			populateEntryProduct((CartModel) source, orderEntry);
					...
					&& orderEntry.getProduct().getBundleTemplates() != null)
			{
				...
				orderEntry.getProduct().getBundleTemplates().stream()
	...
	protected void populateEntryProduct(final CartModel source, final OrderEntryData orderEntry)
	{
		if (orderEntry.getProduct() != null && orderEntry.getProduct().getBundleTemplates() == null)
		{
			final CartEntryModel entryModel = getCartService().getEntryForNumber(source, orderEntry.getEntryNumber());
	 ...
	 * @return
	 */
	 protected int getCartMaxPotentialProductCount(final AbstractOrderData orderData, final BundleTemplateData template)
	 {
		 int cnt = 0;
	...
	protected boolean areComponentsEqual(final OrderEntryData entryData, final BundleTemplateData templateData)
	{
		if (entryData.getComponent() == null || entryData.getBundleNo() != DefaultBundleCommerceCartService.NO_BUNDLE)
		{
			return false;
		}
			...
			final int numberOfPotentialProduct, final List<BundleTemplateData> allBundleTemplates)
	{
		...
		if (target == null || template == null || CollectionUtils.isEmpty(template.getProducts()))
		{
			return potentialEntriesList;
		}
	...
	{
		if (productData == null || template == null || template.getId() == null
				|| CollectionUtils.isEmpty(target.getEntries()))
		{
			return false;
		}

		return target.getEntries().stream()
				.filter(orderEntry -> orderEntry != null)
				...
				.filter(orderEntry -> template.getId().equals(orderEntry.getComponent().getId()))
				.findAny()
				.isPresent();
	}
    ...
    protected ProductBundlePopulator<ProductModel, ProductData> getProductBundlePopulator()
    {
        return productBundlePopulator;
    }

    @Required
    public void setProductBundlePopulator(ProductBundlePopulator<ProductModel, ProductData> productBundlePopulator)
    {
