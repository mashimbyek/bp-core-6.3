...
public class BundleCartPopulatorTest
{
	...
	private final BundleCartPopulator<CartModel, CartData> bundlingCartPopulator = new BundleCartPopulator<>();
	...
	public void testaddEmptyBundleComponents2ToEntries()
	{
		...
		when(bundleTemplateService.getBundleTemplateForCode(eq("Type1"), any(String.class))).thenReturn(rootTemplate);
		final List<BundleTemplateModel> childTemplates = rootTemplate.getChildTemplates();
		for (int i = 0; i < childTemplates.size(); i++) {
			final BundleTemplateModel child = childTemplates.get(i);
			when(bundleCommerceCartService.checkIsComponentEditable(masterCart, child, 1)).thenReturn(Boolean.TRUE);
			when(bundleTemplateService.getBundleTemplateForCode(eq("Type1child" + i), any(String.class)))
					.thenReturn(child);
			when(bundleTemplateService.getLeafComponents(child)).thenReturn(childTemplates);
		}
		final Collection<OrderEntryData> orderEntries = new ArrayList<>();
	...
	public void testGetLastBundlesEmptyChildrenGetAdded()
	{
		...
		when(bundleTemplateService.getBundleTemplateForCode(eq("Type1"), any(String.class))).thenReturn(rootTemplate1);
		final BundleTemplateModel rootTemplate2 = createBundleTemplates("Type2");
		when(bundleTemplateService.getBundleTemplateForCode(eq("Type2"), any(String.class))).thenReturn(rootTemplate2);
		final Collection<OrderEntryData> orderEntries = new ArrayList<>();
		final BundleTemplateData root1 = new BundleTemplateData();
		root1.setId("Type1");
		final BundleTemplateData root2 = new BundleTemplateData();
		root2.setId("Type2");
		for (int i = 0; i < rootTemplate1.getChildTemplates().size(); i++)
		{
			final BundleTemplateModel child = rootTemplate1.getChildTemplates().get(i);
			when(bundleCommerceCartService.checkIsComponentEditable(masterCart, child, 1)).thenReturn(Boolean.TRUE);
			child.setId("Type1child" + i);
			when(bundleTemplateService.getBundleTemplateForCode(eq(child.getId()), any(String.class))).thenReturn(child);
			when(bundleTemplateService.getLeafComponents(child)).thenReturn(rootTemplate1.getChildTemplates());
		}
		final List<BundleTemplateModel> childTemplates = rootTemplate2.getChildTemplates();
		for (int i = 0; i < childTemplates.size(); i++)
		{
			final BundleTemplateModel child = childTemplates.get(i);
			when(bundleCommerceCartService.checkIsComponentEditable(masterCart, child, 2)).thenReturn(Boolean.TRUE);
			child.setId("Type2child" + i);
			when(bundleTemplateService.getBundleTemplateForCode(eq(child.getId()), any(String.class))).thenReturn(child);
			when(bundleTemplateService.getLeafComponents(child)).thenReturn(rootTemplate2.getChildTemplates());
		}
		orderEntries.add(createOrderEntry(1, "Type1child1", root1));
		...
		//
		//@formatter:on
		final Collection<OrderEntryData> modifiedOrderEntries = bundlingCartPopulator.addEmptyBundleComponents(
				orderEntries, masterCart);
	...
	public void testMultiTypeCart()
	{
		...
		when(bundleTemplateService.getBundleTemplateForCode(eq("Type1"), any(String.class))).thenReturn(rootTemplate1);
		when(bundleTemplateService.getBundleTemplateForCode(eq("Type2"), any(String.class))).thenReturn(rootTemplate2);
		final List<BundleTemplateModel> childTemplates1 = rootTemplate1.getChildTemplates();
		for (int i = 0; i < childTemplates1.size(); i++) {
			final BundleTemplateModel child = childTemplates1.get(i);
			when(bundleCommerceCartService.checkIsComponentEditable(masterCart, child, 1)).thenReturn(Boolean.TRUE);
			when(bundleTemplateService.getBundleTemplateForCode(eq("Type1child" + i), any(String.class)))
					.thenReturn(child);
			when(bundleTemplateService.getLeafComponents(child)).thenReturn(childTemplates1);
		}
		final List<BundleTemplateModel> childTemplates2 = rootTemplate2.getChildTemplates();
		for (int i = 0; i < childTemplates2.size(); i++) {
			final BundleTemplateModel child = childTemplates2.get(i);
			when(bundleCommerceCartService.checkIsComponentEditable(masterCart, child, 2)).thenReturn(Boolean.TRUE);
			when(bundleTemplateService.getBundleTemplateForCode(eq("Type2child" + i), any(String.class)))
					.thenReturn(child);
			when(bundleTemplateService.getLeafComponents(child)).thenReturn(childTemplates2);
		}
		final Collection<OrderEntryData> orderEntries = new ArrayList<>();
		final BundleTemplateData root1 = new BundleTemplateData();
		root1.setId("Type1");
	...
	public void testNonEditableShouldNotGetAdded()
	{
		...
		{
			final BundleTemplateModel child = rootTemplate.getChildTemplates().get(i);
			when(bundleCommerceCartService.checkIsComponentEditable(masterCart, child, 1)).thenReturn(Boolean.TRUE);
			when(bundleTemplateService.getBundleTemplateForCode(eq("Type1child" + i), any(String.class)))
					.thenReturn(child);
			when(bundleTemplateService.getLeafComponents(child)).thenReturn(rootTemplate.getChildTemplates());
		}
		when(bundleCommerceCartService.checkIsComponentEditable(masterCart, rootTemplate.getChildTemplates().get(0), 1))
