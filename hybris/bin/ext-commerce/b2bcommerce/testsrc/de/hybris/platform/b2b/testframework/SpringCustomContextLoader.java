...
public class SpringCustomContextLoader
{
		...
		if (testClazz != null)
		{
			...
			else
			{
				...
				if (ContextLoader.class.equals(contextLoaderClass))
				{
					try
					{
						contextLoaderClass = (Class<? extends ContextLoader>) getClass().getClassLoader()
								.loadClass(DEFAULT_CONTEXT_LOADER_CLASS_NAME);
					}
					catch (final ClassNotFoundException ex)
					{
						throw new IllegalStateException("Could not load default ContextLoader class ["
								...
								+ "attribute or make the default loader class available.", ex);
					}
				}
				contextLoader = BeanUtils.instantiateClass(contextLoaderClass);
				locations = retrieveContextLocations(contextLoader, testClazz); // NOPMD FIXME SpringCustomContextLoader in both basecommerce and b2bcommerce
			}
		}
	}

	public void loadApplicationContexts(final GenericApplicationContext globalCtx)
	{
		if (locations == null)
		{
			return;
	...
	/**
	 * Retrieve {@link org.springframework.context.ApplicationContext} resource locations for the supplied {@link Class
	 * class}, using the supplied {@link org.springframework.test.context.ContextLoader} to
	...
	protected String[] retrieveContextLocations(final ContextLoader contextLoader, final Class<?> clazz)
	{
		...
		while (declaringClass != null)
		{
			...
			{
				LOG.trace(
						"Retrieved @ContextConfiguration [" + contextConfiguration + "] for declaring class [" + declaringClass + "]");
			}
