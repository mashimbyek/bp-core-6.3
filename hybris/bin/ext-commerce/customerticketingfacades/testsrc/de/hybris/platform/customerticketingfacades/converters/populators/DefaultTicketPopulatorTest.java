 ...
 */
@UnitTest
public class DefaultTicketPopulatorTest extends ServicelayerTest
{
	@InjectMocks
	...
	public void shouldPopulateTicketData()
	{
		...
		Assert.assertEquals(SUBJECT, ticketData.getSubject());
		Assert.assertEquals(OPEN, ticketData.getStatus().getId());
		Assert.assertEquals(COMPLETED, ticketData.getAvailableStatusTransitions().get(0).getId());
		Assert.assertEquals(yesterday, ticketData.getCreationDate());
		Assert.assertEquals(today, ticketData.getLastModificationDate());
		Assert.assertEquals(TicketCategory.ENQUIRY, ticketData.getTicketCategory());
	}
	...
	public void shouldPopulateOrder()
	{
		...
		Assert.assertEquals("Order: order-code-123; Updated: 11-07-16", ticketData.getAssociatedTo());
	}
	...
	public void shouldPopulateTicketEvent()
	{
		...
		Assert.assertEquals(2, ticketData.getTicketEvents().size());

		Assert.assertEquals(TEXT1, ticketData.getTicketEvents().get(0).getText());
		Assert.assertEquals(Boolean.TRUE, ticketData.getTicketEvents().get(0).getAddedByAgent());
		Assert.assertEquals(TEXT1, ticketData.getTicketEvents().get(0).getText());

		Assert.assertEquals(TEXT2, ticketData.getTicketEvents().get(1).getText());
	...
	public void shouldPopulateTicketAttachment()
	{
		...
		Assert.assertEquals(1, ticketData.getTicketEvents().get(0).getAttachments().size());
		Assert.assertEquals("real-file-name", ticketData.getTicketEvents().get(0).getAttachments().get(0).getFilename());
		Assert.assertEquals("test-url", ticketData.getTicketEvents().get(0).getAttachments().get(0).getURL());
	}
	...
	public void shouldGetHistoryMessage()
	{
		...
		final String messageHistory = populator.getMessageHistory(csTicketModel);

		Assert.assertEquals(
				"11-07-16 12:00 AM added by Customer Service\nTEXT1\n\n10-07-16 12:00 PM added by customer-name\nTEXT2\n\n",
				messageHistory);
	}
}
