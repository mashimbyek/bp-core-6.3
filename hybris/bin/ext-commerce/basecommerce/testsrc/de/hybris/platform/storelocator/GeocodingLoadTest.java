...
public class GeocodingLoadTest extends AbstractGeocodingTest
{
	...
	{
		assertEquals("Initially, we expect all entries to be submitted for geocoding", 35, getPointOfServiceDao().getPosToGeocode()
				.size());
		assertEquals("With batch size limitation:20, dao resulting collection has unexpected size", 20, getPointOfServiceDao()
				.getPosToGeocode(20).size());
		assertEquals("With batch size limitation:100, dao resulting collection has unexpected size", 35, getPointOfServiceDao()
				.getPosToGeocode(100).size());
		assertEquals("With batch size limitation:35, dao resulting collection has unexpected size", 35, getPointOfServiceDao()
				.getPosToGeocode(35).size());

		for (final PointOfServiceModel posModel : getPointOfServiceDao().getPosToGeocode())
		{
		...
		for (final PointOfServiceModel pos : getPointOfServiceDao().getAllPos())
		{
		...
		assertEquals("not geocoded entries amount not as expected", getPointOfServiceDao().getPosToGeocode().size(), notGeocoded.size());
		...
		assertEquals("Not geocoded entries amount should be increased by 5", notGeocoded.size() + 5, getPointOfServiceDao()
		...
		assertEquals("There should not be any not geocoded entries in the end", 0, getPointOfServiceDao().getPosToGeocode().size());
