...
public class GeocodingJobTest extends BaseCommerceBaseTest
{
	 ...
	 *
	 */
	private void createTestCronJob(final Integer batchSize, final Integer internalDelay)
	...
	{
		assertEquals("Initially, we expect all entries to be submitted for geocoding", 5,
				pointOfServiceDao.getPosToGeocode().size());
		assertEquals("With batch size limitation:3, dao resulting collection has unexpected size", 5,
				pointOfServiceDao.getPosToGeocode(10).size());
		assertEquals("With batch size limitation:5, dao resulting collection has unexpected size", 5,
				pointOfServiceDao.getPosToGeocode(5).size());
		...
		assertEquals("Not geocoded entries amount should be increased by 3", notGeocoded.size() + 3,
				pointOfServiceDao.getPosToGeocode().size());
	}
}
