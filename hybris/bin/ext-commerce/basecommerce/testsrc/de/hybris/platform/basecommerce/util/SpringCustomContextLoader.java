...
public class SpringCustomContextLoader
{
	...
	public SpringCustomContextLoader(final Class testClazz)
	{
		...
		else
		{
			...
			if (ContextLoader.class.equals(contextLoaderClass))
			{
				...
				{
					throw new IllegalStateException("Could not load default ContextLoader class [" + DEFAULT_CONTEXT_LOADER_CLASS_NAME
							+ "]. Specify @ContextConfiguration's 'loader' " + "attribute or make the default loader class available.",
							ex);
				}
			}
	...
	protected String[] retrieveContextLocations(final ContextLoader contextLoader, final Class<?> clazz)
	{
		...
		while (declaringClass != null)
		{
			...
			declaringClass = contextConfiguration.inheritLocations() ? AnnotationUtils.findAnnotationDeclaringClass(annotationType,
