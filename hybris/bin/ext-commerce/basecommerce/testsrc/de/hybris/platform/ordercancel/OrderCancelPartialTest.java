...
public class OrderCancelPartialTest extends AbstractOrderCancelTest
{
	...
	public void testAllCancelableOrderEntriesNoConsignments()
	{
		...
		final Map<AbstractOrderEntryModel, Long> cancelableEntries = orderCancelService.getAllCancelableEntries(this.getOrder(),
	...
	public void testAllCancelableOrderEntriesWithConsignments()
	{
		...
		createConsignment(this.getOrder(), "testConsignment1", Arrays.asList(new AbstractOrderEntryModel[]
		{ entry1, entry2 }));

		createConsignment(this.getOrder(), "testConsignment2", Arrays.asList(new AbstractOrderEntryModel[]
		{ entry3 }));
			...
			throws OrderCancelException
	{
		...
		paymentAdapterMock.recalculateOrderAndModifyPayments(getOrder());
		notificationsAdapterMock
				.sendCancelFinishedNotifications(and(capture(capturedArgument), isA(OrderCancelRecordEntryModel.class)));
			...
			throws OrderCancelException
	{
		...
		notificationsAdapterMock
				.sendCancelPendingNotifications(and(capture(capturedArgument), isA(OrderCancelRecordEntryModel.class)));
