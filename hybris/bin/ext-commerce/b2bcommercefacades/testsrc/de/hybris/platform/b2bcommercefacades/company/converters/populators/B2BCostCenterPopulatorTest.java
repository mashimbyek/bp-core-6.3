...
public class B2BCostCenterPopulatorTest
{
	...
	public void shouldPopulate()
	{
		...
		Assert.assertEquals("costCenterData.getCode() should be " + MODEL_CODE, MODEL_CODE, costCenterData.getCode());
		Assert.assertEquals("costCenterData.getName() should be " + costCenter.getName(), costCenter.getName(),
				costCenterData.getName());
		Assert.assertEquals("costCenterData.getCurrency().getIsocode() should be " + CUR_ISOCODE, CUR_ISOCODE, costCenterData
				.getCurrency().getIsocode());
		Assert.assertEquals("costCenterData.getCurrency().getIsocode() should be " + costCenter.getActive(),
				costCenter.getActive(), Boolean.valueOf(costCenterData.isActive()));
		Assert.assertTrue("size of costCenterData.getUnit().getAddresses() should be 1", costCenterData.getUnit().getAddresses()
				.size() == 1);
	}
}
