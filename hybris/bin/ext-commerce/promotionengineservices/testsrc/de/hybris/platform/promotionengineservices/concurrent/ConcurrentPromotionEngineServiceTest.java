...
public class ConcurrentPromotionEngineServiceTest extends PromotionEngineServiceBaseTest
{
	...
	public void testConcurrentPromotionUpdate() throws Exception
	{
		...
		assertEquals("wrong discount value", 19.0, discountValue.getValue(), 0.00);
		assertFalse("discount should not be absolute", discountValue.isAbsolute());
		...
		assertEquals("percentageDiscountCameraAccessories.drlPromotion", promoResult1.getPromotion().getCode());
	...
	protected void initializeRuleEngine(final AbstractRuleEngineRuleModel... rules)
	{
		...
		final RuleEngineActionResult result = commerceRuleEngineService
				.initialize(ruleEngineTestSupportService.getTestRulesModule(abstractContext, stream(rules).collect(toSet())));
