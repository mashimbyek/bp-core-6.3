...
public class DefaultCatConditionResolutionStrategyUnitTest
{
	...
	public void testCleanStoredParameterValues()
	{
		...
		when(promotionSourceRuleDao.findAllCatForPromotionSourceRule(rule)).thenReturn(catsForRule);
		...
		when(promotionSourceRuleDao.findAllCombinedCatsForRule(rule)).thenReturn(combinedCatsForRule);
		...
		when(promotionSourceRuleDao.findAllExcludedProductForPromotionSourceRule(rule)).thenReturn(excludedProductsForRule);
		...
		verify(promotionSourceRuleDao, times(1)).findAllCatForPromotionSourceRule(rule);
		verify(modelService, times(1)).removeAll(catsForRule);

		verify(promotionSourceRuleDao, times(1)).findAllCombinedCatsForRule(rule);
		verify(modelService, times(1)).removeAll(combinedCatsForRule);

		verify(promotionSourceRuleDao, times(1)).findAllExcludedProductForPromotionSourceRule(rule);
		verify(modelService, times(1)).removeAll(excludedProductsForRule);

		verify(promotionSourceRuleDao, times(1)).findAllExcludedCatForPromotionSourceRule(rule);
	}
