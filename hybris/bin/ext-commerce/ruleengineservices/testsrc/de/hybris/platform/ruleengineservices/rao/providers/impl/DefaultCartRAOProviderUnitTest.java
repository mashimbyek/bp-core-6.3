...
public class DefaultCartRAOProviderUnitTest
{
	...
	public void testExpandFactModelDiscounts()
	{
		...
		final List discountsList = facts.stream().filter(f -> f instanceof DiscountValueRAO).collect(Collectors.toList());
	...
	public void testExpandFactModelExpandEntries()
	{
		...
		cartRAO.setCode(cartContext.getCartModel().getCode());
		assertThat(facts).isNotEmpty().contains(cartRAO);
		final List entriesList = facts.stream().filter(f -> f instanceof OrderEntryRAO).collect(Collectors.toList());
	...
	public void testExpandFactModelExpandProducts()
	{
		...
		cartRAO.setCode(cartContext.getCartModel().getCode());
		assertThat(facts).isNotEmpty().contains(cartRAO);
		final List productsList = facts.stream().filter(f -> f instanceof ProductRAO).collect(Collectors.toList());
	...
	public void testExpandFactModelExpandCategories()
	{
		...
		cartRAO.setCode(cartContext.getCartModel().getCode());
		assertThat(facts).isNotEmpty().contains(cartRAO);
		final List categoryList = facts.stream().filter(f -> f instanceof CategoryRAO).collect(Collectors.toList());
