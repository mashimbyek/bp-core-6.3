...
public class DefaultAlipayPaymentTransactionStrategy implements AlipayPaymentTransactionStrategy
{
	...
	{
		List<AlipayPaymentTransactionEntryModel> entryList = getPaymentTransactionEntryByType(orderModel, status,
	...
	@Override
	public Map<OrderModel, Boolean> updateForRefundNotification(List<AlipayRefundData> alipayRefundData)
	{

		Map<OrderModel, Boolean> refundStatus = new HashMap<OrderModel, Boolean>();
		for (AlipayRefundData refundData : alipayRefundData)
		{
			...
			OrderModel order = (OrderModel) refundTransaction.getOrder();

			List<AlipayPaymentTransactionEntryModel> successRefundEntryList = alipayPaymentTransactionEntryDao
					.findPaymentTransactionEntryByTypeAndStatus(PaymentTransactionType.REFUND_STANDALONE, TransactionStatus.ACCEPTED,
							refundTransaction);

			if (successRefundEntryList.size() > 0)
			{
				continue;
			}

			List<AlipayPaymentTransactionEntryModel> refundEntries = alipayPaymentTransactionEntryDao
					.findPaymentTransactionEntryByTypeAndStatus(PaymentTransactionType.REFUND_STANDALONE, TransactionStatus.REVIEW,
							refundTransaction);

			if (refundEntries.size() == 0)
			{
				continue;
			}

			AlipayPaymentTransactionEntryModel refundEntry = refundEntries.get(0);
			...
			refundEntry
					.setTransactionStatusDetails(refundData.getPayerRefundStatus() + "; Refund Batch No: " + refundData.getBatchNo());
	...
	{
		Optional<AlipayPaymentTransactionModel> result = getPaymentTransactionWithCaptureEntry(orderModel,
				TransactionStatus.ACCEPTED);
		result.ifPresent(alipayPaymentTransactionModel -> {
			AlipayPaymentTransactionEntryModel alipayPaymentTransactionEntryModel = new AlipayPaymentTransactionEntryModel();
			...
			alipayPaymentTransactionEntryModel.setTransactionStatusDetails(
					TransactionStatusDetails.SUCCESFULL.name() + "; Refund request: " + alipayRefundRequestData.getBatchNo());
			...
			final TransactionStatus status, final PaymentTransactionType paymentTransactionType)
	{
		for (final PaymentTransactionModel transaction : orderModel.getPaymentTransactions())
		{
			if (transaction instanceof AlipayPaymentTransactionModel)
			{
				...
				{
					List<AlipayPaymentTransactionEntryModel> entryList = alipayPaymentTransactionEntryDao
			...
			final TransactionStatus status, final String alipayCode)
	{
		...
		Optional<AlipayPaymentTransactionModel> result = getPaymentTransactionWithCaptureEntry(orderModel, status);
		if (result.isPresent())
		{
			...
			final AlipayPaymentTransactionEntryModel alipayPaymentTransactionEntry)
	{
		...
		if (alipayRawPaymentStatus.getFlagTradeLocked() != null)
		{
			alipayPaymentTransactionEntry
					.setLocked("0".equals(alipayRawPaymentStatus.getFlagTradeLocked().trim()) ? Boolean.TRUE : Boolean.FALSE);
		}
		if (alipayRawPaymentStatus.getUseCoupon() != null)
		{
			alipayPaymentTransactionEntry
					.setCouponUsed("T".equals(alipayRawPaymentStatus.getUseCoupon().trim()) ? Boolean.TRUE : Boolean.FALSE);
		}
			...
			final AlipayPaymentTransactionEntryModel alipayPaymentTransactionEntry)
	{
		...
		{
			alipayPaymentTransactionEntry
					.setCouponUsed("T".equals(directPayNotifyResponseData.getUseCoupon().trim()) ? Boolean.TRUE : Boolean.FALSE);
		}
