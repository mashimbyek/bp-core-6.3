...
/**
 * Concrete implementation of the customer list search strategy for getting a list of customers
 * that have a consignment for a specific delivery PointOfService in a specific state.
 *
 * In other words it finds customers that are about to pickup an item from a specific store.
 *
 * The strategy will first query for groups for the supplied employee Uid.
 * Second step will be making sure that these groups have stores associated to them, then collecting that stores and calling:
...
public class DefaultBopisCustomerListSearchStrategy implements CustomerListSearchStrategy
{
	...
	private final static Logger LOG = Logger.getLogger(DefaultBopisCustomerListSearchStrategy.class);
			...
			final PageableData pageableData, final Map<String, Object> parameterMap)
	{
		...
		if (CollectionUtils.isNotEmpty(storeEmployeeGroups))
		{
			...
			final List<PointOfServiceModel> filteredPointOfServices = storeEmployeeGroups.stream()
					.filter(storeEmployee -> storeEmployee.getStore() != null).map(StoreEmployeeGroupModel::getStore).collect(Collectors.toList());
