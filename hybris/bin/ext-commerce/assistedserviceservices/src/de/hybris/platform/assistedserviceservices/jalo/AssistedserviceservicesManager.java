 ...
 */
public class AssistedserviceservicesManager extends GeneratedAssistedserviceservicesManager
{
	/** Edit the local|project.properties to change logging behavior (properties 'log4j.*'). */
	private static final Logger LOG = Logger.getLogger(AssistedserviceservicesManager.class.getName());
	 ...
	 */
	public static AssistedserviceservicesManager getInstance()
	{
		return (AssistedserviceservicesManager) Registry.getCurrentTenant().getJaloConnection().getExtensionManager().getExtension(
				AssistedserviceservicesConstants.EXTENSIONNAME);
	}
	 ...
	 */
	public AssistedserviceservicesManager() // NOPMD 
	{
		if (LOG.isDebugEnabled())
		{
			LOG.debug("constructor of AssistedserviceservicesManager called.");
		}
	}
	...
	public void init()
	{
		if (LOG.isDebugEnabled())
		{
			LOG.debug("init() of AssistedserviceservicesManager called. " + getTenant().getTenantID());
		}
	}
	...
	public void destroy()
	{
		if (LOG.isDebugEnabled())
		{
			LOG.debug("destroy() of AssistedserviceservicesManager called, current tenant: " + getTenant().getTenantID());
		}
	}
