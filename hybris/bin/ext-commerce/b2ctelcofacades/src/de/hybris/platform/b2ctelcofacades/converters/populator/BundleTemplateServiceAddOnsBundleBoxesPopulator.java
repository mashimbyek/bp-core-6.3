 ...
 */
public class BundleTemplateServiceAddOnsBundleBoxesPopulator<
		SOURCE extends BundleTemplateServiceAddOnsPopulatorParameters, TARGET extends BundleTemplateData>
		implements Populator<SOURCE, TARGET>
{
	...
	public void populate(final SOURCE source, final TARGET target) throws ConversionException
	{
		...
		AtomicBoolean beforeExpanded = new AtomicBoolean(true);
		final List<BundleBoxData> bundleBoxDatas = childBundleTemplates.stream()
				.filter(bundleTemplateModel -> getGuidedSellingService().isComponentToBeDisplayedOnGuidedSellingSelectPage(
						cart, bundleTemplateModel, bundleNo, ServiceAddOnModel.class))
				.map(bundleTemplateModel -> constructBundleBoxData(
						source, cart, expandedComponent, beforeExpanded, bundleTemplateModel))
				.collect(Collectors.toList());
		target.setBundleBoxes(bundleBoxDatas);
	}

	protected BundleBoxData constructBundleBoxData(final SOURCE source, final CartModel cart,
			final BundleTemplateModel expandedComponent, final AtomicBoolean beforeExpanded,
			final BundleTemplateModel bundleTemplateModel)
	{
		...
		{
			populateBundleBoxEntryForDisabledProducts(
					cart, source.getBundleNo().intValue(), bundleTemplateModel, boxEntryData);
