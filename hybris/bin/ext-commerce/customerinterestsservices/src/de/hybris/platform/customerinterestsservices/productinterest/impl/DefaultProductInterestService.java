 ...
 *
 */
public class DefaultProductInterestService implements ProductInterestService
{
	...
	public void removeProductInterestByProduct(final String productCode)
	{
		...
		productInterestDao.findProductInterestsByCustomer(currentCustomer, baseStoreModel, baseSiteModel).stream()
				.filter(x -> productCode.equals(x.getProduct().getCode()))
	...
	public Map<ProductModel, Map<NotificationType, Date>> getProductsByCustomerInterests(final PageableData pageableData)
	{
		...
		productMap.forEach((productPk, interestCreationMap) -> {
			fillProductModelMap(productModelMap, productPk, interestCreationMap);
			...
			final String productPk, final Map<String, String> interestCreationMap)
	{
		try
		{
			final ProductModel productModel = (ProductModel) modelService.get(PK.parse(productPk));
	...
	{
		final Map<NotificationType, Date> interestMap = new LinkedHashMap<>();
		...
		interestCreationMap.forEach((notificationTypePK, creationTime) -> {
			...
			{
				LOG.info("Can't parse " + creationTime + " Date format");
			}
			catch (final ModelLoadingException e)
			{
				LOG.info("No NotificationType found for given pk " + notificationTypePK);
			}
		});
		return interestMap;
	}
