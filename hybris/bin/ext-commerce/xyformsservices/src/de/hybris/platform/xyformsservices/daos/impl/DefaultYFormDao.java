...
{
	private static final String QUERY_FORMDATA_BY_APPLICATIONID_FORMID_REFID = "SELECT {formData." + YFormDataModel.PK + "} FROM {"
			+ YFormDataModel._TYPECODE + " AS formData}, {" + YFormDefinitionModel._TYPECODE + " AS formDefinition}"
	...
	private static final String QUERY_LATEST_FORMDEFINITION = "SELECT {formDefinition." + YFormDefinitionModel.PK + "} " + "from {"
			+ YFormDefinitionModel._TYPECODE + " as formDefinition}" + " WHERE {formDefinition." + YFormDefinitionModel.APPLICATIONID
			+ "} = ?applicationId and " + " {formDefinition." + YFormDefinition.FORMID + "} = ?formId and " + "{formDefinition."
			+ YFormDefinitionModel.VERSION + "} = ({{SELECT MAX({fd." + YFormDefinitionModel.VERSION + "}) from {"
			+ YFormDefinitionModel._TYPECODE + " as fd} WHERE {fd." + YFormDefinitionModel.APPLICATIONID
			+ "} = ?applicationId AND {fd." + YFormDefinitionModel.FORMID + "} = ?formId }})";
