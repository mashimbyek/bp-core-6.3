...
public class AbstractOrderPopulatorTest
{
	...
	{
		given(cartModel.getTotalPrice()).willReturn(PRICE);
	...
	{
		given(cartModel.getTotalPrice()).willReturn(0.0d);
		given(cartModel.getNet()).willReturn(true);
		Assert.assertEquals(0.0d, cartPopulator.calcTotalWithTax(cartModel).doubleValue(), EPSILON);
	}
	...
	public void shouldAddTaxToOrder()
	{
		given(cartModel.getTotalPrice()).willReturn(PRICE);
		given(cartModel.getNet()).willReturn(true);
		given(cartModel.getTotalTax()).willReturn(TAX);
