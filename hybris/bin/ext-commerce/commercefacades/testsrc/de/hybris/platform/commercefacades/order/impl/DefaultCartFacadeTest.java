...
public class DefaultCartFacadeTest
{
	...
	public void testAddToCart() throws CommerceCartModificationException
	{
		...
		defaultCartFacade.addToCart("prodCode", 1);
	}
	...
	public void testUpdateCartEntry() throws CommerceCartModificationException
	{
		...
		given(cartService.getSessionCart()).willReturn(cartModel);
		given(productService.getProductForCode(anyString())).willReturn(product);
		defaultCartFacade.updateCartEntry(0, 1);
	}
	...
	public void testConfigureCartEntry() throws CommerceCartModificationException
	{
		...
		final CommerceCartParameter cartParameter = captor.getValue();
		final List<ProductConfigurationItem> configurationItems = new ArrayList<>(cartParameter.getProductConfiguration());
		Assert.assertEquals(cartModel, cartParameter.getCart());
		Assert.assertEquals(RADIOBUTTON_CONFIGURATOR_TYPE, cartParameter.getProductConfiguration().iterator().next()
				.getConfiguratorType());
	...
	public void testConfigureCartEntryMultipleConfigurators() throws CommerceCartModificationException
	{
		...
		final List<CommerceCartParameter> cartParameters = captor.getAllValues();

		Assert.assertEquals(1, cartParameters.size());
		Assert.assertEquals(cartModel, cartParameters.get(0).getCart());
		Assert.assertEquals(RADIOBUTTON_CONFIGURATOR_TYPE, cartParameters.get(0).getProductConfiguration().iterator().next()
				.getConfiguratorType());
	...
	public void testUpdateCartEntryWithEmptyConfiguration() throws CommerceCartModificationException {
		...
		CartModificationData result = defaultCartFacade.updateCartEntry(orderEntryData);
