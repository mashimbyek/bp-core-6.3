...
public class DefaultCustomerListFacadeTest
{
	...
	private final String implementationType = "DUMMY_IMPL";
	@Mock
	...
	public void setUp()
	{
		...
		given(customerListSearchService.getPagedCustomers("Actual Value", TEST_EMPLOYEE_ID, pageableData, null))
				.willReturn(searchPageData);

		given(userGroupConverter.convertAll(customerListModels)).willReturn(userGroupDataList);

		given(customerListService.getCustomerListForEmployee("Actual Value", TEST_EMPLOYEE_ID)).willReturn(customerListModel3);
	}
	...
	{
		defaultCustomerListFacade.getPagedCustomersForCustomerListUID("Actual Value", TEST_EMPLOYEE_ID, pageableData, null);
