...
public class GroupOrderConsignmentEntryPopulator extends GroupOrderEntryPopulator<OrderModel, OrderData>
{
	...
	public void populate(final OrderModel source, final OrderData target) throws ConversionException
	{
		...
		{
			consignment.setEntries(groupConsignmentEntries(consignment.getEntries()));
		}

		target.setUnconsignedEntries(super.groupEntries(target.getUnconsignedEntries()));
	}

	protected List<ConsignmentEntryData> groupConsignmentEntries(final List<ConsignmentEntryData> entries)
	{
		...
		for (final ConsignmentEntryData consignmentEntry : entries)
		{
			...
			if (isGroupable(product))
			{
				...
				if (!baseProductGroupConsignmentEntryMap.containsKey(product.getBaseProduct()))
				{
					final ConsignmentEntryData newGroupConsignmentEntry = new ConsignmentEntryData();
