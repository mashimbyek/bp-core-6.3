...
public abstract class AbstractQuotationService implements QuotationService
{
	...
	public Double getPrice(final QuotationResponseData responseData, final String itemId, final boolean isPaynow)
	{
		...
		if (quotationItem != null)
		{
			...
			{
				payNowPrice = StringUtils.isNotEmpty(quotationItem.getRecurringPrice())
						? new Double(quotationItem.getRecurringPrice()) : payNowPrice;
			}
		}
	...
	protected Map<String, String> populateItemProperteis(final String itemId)
	{
		...
		if (product instanceof SubscriptionProductModel)
		{
			...
			final SubscriptionPricePlanModel pricePlan = getSubscriptionCommercePriceService()
					.getSubscriptionPricePlanForProduct(subscriptionProduct);

			if (subscriptionProduct.getSubscriptionTerm() != null
					&& subscriptionProduct.getSubscriptionTerm().getBillingPlan() != null)
			{
				coverageProperties.put(TERM_FREQUENCY,
						subscriptionProduct.getSubscriptionTerm().getBillingPlan().getBillingFrequency().getCode());
			}
			...
			{
				coverageProperties.put(oneTimeChargeEntry.getBillingEvent().getCode(),
						Double.toString(oneTimeChargeEntry.getPrice()));
			}
		}
