...
public class RuleEngineRuleRemoveInterceptorUnitTest
{
	...
	public void setUp()
	{
		...
		when(engineRuleDao.getRuleByCodeAndMaxVersion(RULE_CODE, 0l)).thenReturn(previousVersionModel);
	}
	...
	{
		when(engineRuleDao.getRuleByCodeAndMaxVersion(RULE_CODE, 0L)).thenReturn(null);
		removeInterceptor.onRemove(model, context);
		verify(ruleEngineRemoveHandler, times(1)).handleOnRemove(model, context);
