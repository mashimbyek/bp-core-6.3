...
public class PolicyCreationSubmitOrderStrategy implements SubmitOrderStrategy
{
	...
	public void submitOrder(final OrderModel order)
	{
		...
		final PolicyRequestData requestData = buildPolicyRequestData(order);
		final PolicyResponseData responseData = getPolicyServiceFacade().requestPolicyCreation(requestData);

		if (responseData.getItems() != null)
		{
			for (final PolicyItemResponseData policyItemResponseData : responseData.getItems())
			{
