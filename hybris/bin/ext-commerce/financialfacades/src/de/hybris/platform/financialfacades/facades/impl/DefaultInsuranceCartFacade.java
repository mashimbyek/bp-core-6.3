...
public class DefaultInsuranceCartFacade extends DefaultBundleCartFacade implements InsuranceCartFacade
{
	...
	public CommerceSaveCartResultData saveCurrentUserCart() throws CommerceSaveCartException
	{
		...
		{
			final CommerceSaveCartResultData commerceSaveCartResultData = getSaveCartFacade()
					.saveCart(new CommerceSaveCartParameterData());
		...
		{
			LOG.debug(
					"No need to save cart, because is anonymous user/no session cart/cart no entries/no quote/already saved cart.");
		}
