...
public class LifeQuotationPricingStrategy extends AbstractQuotationPricingStrategy
{
	...
	{
		return LIFE_VALUE_JOIN_APPLICANTS
				.equalsIgnoreCase(MapUtils.getString(requestData.getProperties(), FinancialservicesConstants.LIFE_WHO_COVERED));
	}

	protected boolean isMainSmoke(final QuotationRequestData requestData)
	{
		return LIFE_VALUE_CURRENT_SMOKER
				.equalsIgnoreCase(MapUtils.getString(requestData.getProperties(), FinancialservicesConstants.LIFE_MAIN_SMOKE));
	}

	protected boolean isSecondSmoke(final QuotationRequestData requestData)
	{
		return isJointApplicant(requestData) && LIFE_VALUE_CURRENT_SMOKER
				.equalsIgnoreCase(MapUtils.getString(requestData.getProperties(), FinancialservicesConstants.LIFE_SECOND_SMOKE));
	}
