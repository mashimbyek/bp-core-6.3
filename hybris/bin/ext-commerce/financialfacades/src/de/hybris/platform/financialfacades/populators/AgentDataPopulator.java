...
public class AgentDataPopulator<SOURCE extends AgentModel, TARGET extends AgentData> implements Populator<SOURCE, TARGET>
{
	...
	public void populate(final SOURCE agentModel, final TARGET agentData) throws ConversionException
	{
		...
		if (categoryModels != null)
		{
			...
			{
				final CategoryData converted = getCategoryConverter().convert(category);
		...
		{
			final AddressData addressData = getAddressConverter().convert(agentModel.getEnquiry());
	...
	protected ImageData createImageData(final SOURCE source, final String imageFormat, final ImageDataType type)
	{
