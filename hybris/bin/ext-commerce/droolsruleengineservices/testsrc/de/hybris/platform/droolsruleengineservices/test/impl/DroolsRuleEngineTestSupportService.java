...
public class DroolsRuleEngineTestSupportService implements RuleEngineTestSupportService
{
	...
	{
		final Set<DroolsRuleModel> droolsRules = rules.stream().map(r -> (DroolsRuleModel) r).collect(toSet());

		final DroolsKIEBaseModel rulesBase = getModelService().create(DroolsKIEBaseModel.class);
		rulesBase.setName(moduleName);
		rulesBase.setRules(droolsRules);

		final DroolsKIEModuleModel rulesModule = getModelService().create(DroolsKIEModuleModel.class);
