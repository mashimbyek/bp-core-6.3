...
public class DefaultCommerceQuoteDao extends DefaultGenericDao<QuoteModel> implements CommerceQuoteDao
{
	private static final String FIND_QUOTES_BY_CUSTOMER_STORE_CODE_MAX_VERSION_QUERY = "SELECT {q1:" + QuoteModel.PK + "} FROM {"
			...
			+ "} = ?" + QuoteModel.USER + " AND {q1:" + QuoteModel.STORE + "} = ?" + QuoteModel.STORE
			+ " AND EXISTS ( SELECT CODE_MAX_VERSION.code, CODE_MAX_VERSION.maxVersion FROM ( {{ SELECT {" + QuoteModel.CODE
			+ "} as code, max({" + QuoteModel.VERSION + "}) as maxVersion FROM {" + QuoteModel._TYPECODE + "} WHERE {"
			+ QuoteModel.STATE + "} IN (?quoteStates)" + " AND {" + QuoteModel.USER + "} = ?" + QuoteModel.USER + " AND {"
			+ QuoteModel.STORE + "} = ?" + QuoteModel.STORE + " GROUP BY {" + QuoteModel.CODE + "} }} ) CODE_MAX_VERSION WHERE {q1:"
			+ QuoteModel.CODE + "} = CODE_MAX_VERSION.code AND {q1:" + QuoteModel.VERSION + "} = CODE_MAX_VERSION.maxVersion )";
	...
	private static final String FIND_QUOTE_BY_CUSTOMER_STORE_CODE_MAX_VERSION_QUERY = "SELECT {q1:" + QuoteModel.PK + "} FROM {"
			...
			+ QuoteModel.CODE + "} = ?" + QuoteModel.CODE + "  ORDER BY {q1:" + QuoteModel.VERSION + "} DESC";
			...
			final String quoteCode, final Set<QuoteState> quoteStates)
	{
		...
		final FlexibleSearchQuery flexibleSearchQuery = new FlexibleSearchQuery(FIND_QUOTE_BY_CUSTOMER_STORE_CODE_MAX_VERSION_QUERY);
