...
public class DefaultCommerceQuoteService implements CommerceQuoteService
{
			...
			final UserModel userModel)
	{
		...
		return (updatedQuoteModel.getCartReference() != null) ? updatedQuoteModel.getCartReference() : getCartService()
				.createCartFromQuote(updatedQuoteModel);
	}
	...
	public QuoteModel submitQuote(final QuoteModel quoteModel, final UserModel userModel)
	{
		...
		QuoteModel updatedQuoteModel = isSessionQuoteSameAsRequestedQuote(quoteModel) ? updateQuoteFromCart(getCartService()
				.getSessionCart(), userModel) : quoteModel;
	...
	protected QuoteModel createQuoteSnapshot(final QuoteModel quoteModel)
	{
		...
		{
			throw new IllegalArgumentException(String.format("Unable to create Quote Snapshot for Quote [Quote Code : %s],"
					+ " because Snapshot transition state was not found for current quote state : %s ", quoteModel.getCode(),
					currentQuoteState));
		}
	...
	public CartModel acceptAndPrepareCheckout(final QuoteModel quoteModel, final UserModel userModel)
	{
		...
		if (getQuoteExpirationTimeValidationStrategy().hasQuoteExpired(quoteModel))
		{
			...
			throw new CommerceQuoteExpirationTimeException(String.format("Quote has expired. Quote Code : [%s].",
					expiredQuoteModel.getCode()));
		}
			...
			final Double discountRate, final DiscountType discountType)
	{
		...
		boolean isCalculationRequired = CollectionUtils.isNotEmpty(getOrderQuoteDiscountValuesAccessor().getQuoteDiscountValues(
				abstractOrderModel));
			...
			final AbstractOrderModel abstractOrderModel)
	{
		...
		if (DiscountType.PERCENT.equals(discountType)
				&& (DoubleMath.fuzzyCompare(rate, 100, EPSILON) > 0 || DoubleMath.fuzzyCompare(rate, 0, EPSILON) < 0))
		{
			throw new IllegalArgumentException("Discount type is percent, but the discount rate is not in the range of 0 to 100!");
		}
	...
	public void cancelQuote(final QuoteModel quoteModel, final UserModel userModel)
	{
		...
		getEventService().publishEvent(
				new QuoteCancelEvent(quoteToCancel, userModel, getQuoteUserTypeIdentificationStrategy().getCurrentQuoteUserType(
						userModel).get()));
	}
			...
			final BaseStoreModel store)
	{
		...
		validateParameterNotNullStandardMessage("quoteUserModel", quoteUserModel);
		validateParameterNotNullStandardMessage("store", store);
		return getUserService().isAnonymousUser(quoteUserModel) ? Integer.valueOf(0) : getCommerceQuoteDao()
				.getQuotesCountForCustomerAndStore(customerModel, store,
						getQuoteStateSelectionStrategy().getAllowedStatesForAction(QuoteAction.VIEW, quoteUserModel));
	}
