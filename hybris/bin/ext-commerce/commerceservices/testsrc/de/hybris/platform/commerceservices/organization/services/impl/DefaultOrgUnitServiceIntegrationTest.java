...
public class DefaultOrgUnitServiceIntegrationTest extends BaseCommerceBaseTest
{
	...
	@Rule
	public ExpectedException thrown = ExpectedException.none(); // NOPMD
	...
	{
		final OrgUnitModel northAmerica = getUnit(NA_UNIT_UID);
		...
		Assert.assertTrue("Expected parent unit not set.", northAmerica.getGroups().contains(getUnit(ROOT_UNIT_UID)));
	}
	...
	protected void validateCreateUnit()
	{
		...
		final OrgUnitModel rootUnit = getUnit(ROOT_UNIT_UID);

		defaultOrgUnitService
				.createUnit(createOrgUnitParam(null, rootUnit, EU_UNIT_UID, EU_UNIT_NAME, Boolean.TRUE, EU_UNIT_DESCRIPTION));

		final OrgUnitModel europe = getUnit(EU_UNIT_UID);
	...
	public void shouldNotCreateUnitForOrgManager()
	{
		...
		defaultOrgUnitService.createUnit(
				createOrgUnitParam(null, getUnit(ROOT_UNIT_UID), EU_UNIT_UID, EU_UNIT_NAME, Boolean.TRUE, EU_UNIT_DESCRIPTION));
	}
	...
	{
		defaultOrgUnitService
				.createUnit(createOrgUnitParam(null, getUnit(ROOT_UNIT_UID), null, EU_UNIT_NAME, Boolean.TRUE, EU_UNIT_DESCRIPTION));
	}
	...
	{
		defaultOrgUnitService
				.createUnit(createOrgUnitParam(null, getUnit(ROOT_UNIT_UID), EU_UNIT_UID, null, Boolean.TRUE, EU_UNIT_DESCRIPTION));
	}
	...
	{
		defaultOrgUnitService
				.createUnit(createOrgUnitParam(null, getUnit(ROOT_UNIT_UID), EU_UNIT_UID, EU_UNIT_NAME, null, EU_UNIT_DESCRIPTION));
	}
	...
	public void shouldUpdateUnit()
	{
		...
		final OrgUnitModel montreal = getUnit(MT_UNIT_UID);
		// retrieve the current parent unit
		final OrgUnitModel canada = getParentUnit(montreal);


		defaultOrgUnitService.updateUnit(
				createOrgUnitParam(montreal, canada, UPDATED_UNIT_UID, UPDATED_UNIT_NAME, Boolean.FALSE, UPDATED_UNIT_DESCRIPTION));

		// retrieve the updated unit to do assertions
		final OrgUnitModel updatedUnit = getUnit(UPDATED_UNIT_UID);
		...
		Assert.assertTrue("Expected parent unit not set.", updatedUnit.getGroups().contains(canada));
	}
	...
	public void shouldUpdateUnitAndAddParent()
	{
		...
		final OrgUnitModel californiaOrgUnit = getUnit(CAL_UNIT_UID);
		final Optional<OrgUnitModel> actualParentUnitOptional = defaultOrgUnitService.getParent(californiaOrgUnit);
		final OrgUnitModel newParent = getUnit(US_UNIT_UID);
		...
		updateParameter.setOrgUnit(californiaOrgUnit);
		updateParameter.setParentUnit(newParent);

		defaultOrgUnitService.updateUnit(updateParameter);

		// retrieve the updated unit to do assertions
		final OrgUnitModel updatedUnit = getUnit(CAL_UNIT_UID);
	...
	public void shouldUpdateUnitAndRemoveParent()
	{
		...
		final OrgUnitModel usaOrgUnit = getUnit(US_UNIT_UID);
		final OrgUnitModel actualParent = getParentUnit(usaOrgUnit);
		...
		// retrieve the updated unit to do assertions
		final OrgUnitModel updatedUnit = getUnit(US_UNIT_UID);
	...
	public void shouldUpdateUnitWithoutUpdatingParentForOrgManager()
	{
		...
		final OrgUnitModel actualParent = getParentUnit(montreal);

		defaultOrgUnitService.updateUnit(createOrgUnitParam(montreal, actualParent, UPDATED_UNIT_UID, UPDATED_UNIT_NAME,
				Boolean.FALSE, UPDATED_UNIT_DESCRIPTION));
	...
	public void shouldNotUpdateUnitWithUpdatingParentForOrgManager()
	{
		...
		defaultOrgUnitService.updateUnit(createOrgUnitParam(montreal, newParent, UPDATED_UNIT_UID, UPDATED_UNIT_NAME, Boolean.FALSE,
	...
	public void shouldNotUpdateUnitForOrgEmployee()
	{
		...
		defaultOrgUnitService.updateUnit(
				createOrgUnitParam(montreal, null, UPDATED_UNIT_UID, UPDATED_UNIT_NAME, Boolean.FALSE, UPDATED_UNIT_DESCRIPTION));
	}
	...
	public void shouldUpdateUnitAndChangeParent()
	{
		...
		defaultOrgUnitService.createUnit(createOrgUnitParam(null, canada, QC_UNIT_UID, "Quebec", Boolean.TRUE, "Unit for Quebec"));

		// retrieve the new parent unit
		final OrgUnitModel quebec = getUnit(QC_UNIT_UID);

		// update the unit
		final OrgUnitParameter updateParameter = createOrgUnitParam(montreal, canada, UPDATED_UNIT_UID, UPDATED_UNIT_NAME,
				Boolean.FALSE, UPDATED_UNIT_DESCRIPTION);
	...
	public void shouldNotActivateUnitForOrgEmployee()
	{
		...
		defaultOrgUnitService.activateUnit(getUnit(CA_UNIT_UID));
	}
	...
	public void shouldDeactivateUnitAlsoDeactiveChildUnit()
	{
		...
		OrgUnitModel northAmerica = getParentUnit(usa);
		...
		northAmerica = getUnit(NA_UNIT_UID);
	...
	public void shouldNotDeactivateUnitForOrgEmployee()
	{
		...
		defaultOrgUnitService.deactivateUnit(getUnit(US_UNIT_UID));
	}
	...
	protected void validateAddMembers()
	{
		...
		defaultOrgUnitService
				.addMembers(createOrgUnitMemberParameter(NA_UNIT_UID, employees, EmployeeModel.class, createPageableData(0, 5)));
	...
	public void shouldNotAddMembersForOrgEmployee()
	{
		...
		defaultOrgUnitService.addMembers(
				createOrgUnitMemberParameter(CA_UNIT_UID, Collections.emptySet(), EmployeeModel.class, createPageableData(0, 5)));
	}
	...
	{
		defaultOrgUnitService.addMembers(
				createOrgUnitMemberParameter(null, Collections.emptySet(), EmployeeModel.class, createPageableData(0, 5)));
	}
	...
	{
		defaultOrgUnitService
				.addMembers(createOrgUnitMemberParameter(NA_UNIT_UID, null, EmployeeModel.class, createPageableData(0, 5)));
	}
	...
	protected void validateRemoveMembers()
	{
		...
		final OrgUnitModel usa = getUnit(US_UNIT_UID);
		...
		defaultOrgUnitService
				.removeMembers(createOrgUnitMemberParameter(US_UNIT_UID, employees, EmployeeModel.class, createPageableData(0, 5)));
	...
	public void shouldNotRemoveMembersForOrgEmployee()
	{
		...
		defaultOrgUnitService.removeMembers(
				createOrgUnitMemberParameter(US_UNIT_UID, Collections.emptySet(), EmployeeModel.class, createPageableData(0, 5)));
	}
	...
	{
		defaultOrgUnitService
				.removeMembers(createOrgUnitMemberParameter(NA_UNIT_UID, null, EmployeeModel.class, createPageableData(0, 5)));
	}
	...
	{
		final SearchPageData<EmployeeModel> searchPageData = defaultOrgUnitService.getMembers(
	...
	{
		defaultOrgUnitService
				.getMembers(createOrgUnitMemberParameter(NA_UNIT_UID, Collections.emptySet(), EmployeeModel.class, null));
	}
	...
	{
		defaultOrgUnitService
				.getMembers(createOrgUnitMemberParameter(NA_UNIT_UID, Collections.emptySet(), null, createPageableData(0, 5)));
	}
	...
	{
		final OrgUnitModel northAmerica = getUnit(NA_UNIT_UID);
		final OrgUnitModel parentUnit = getParentUnit(northAmerica);
	...
	public void shouldNotGetParentForNonOrgEmpoyee()
	{
		...
		thrown.expect(IllegalStateException.class);
		thrown.expectMessage("Not allowed to view. User: " + UNASSIGNED_EMPLOYEE);
		defaultOrgUnitService.getParent(getUnit(NA_UNIT_UID));
	}
	...
	protected OrgUnitModel getUnit(final String uid)
	{
	...
	protected OrgUnitParameter createOrgUnitParam(final OrgUnitModel orgUnit, final OrgUnitModel parentUnit, final String uid,
	...
	<T extends PrincipalModel> OrgUnitMemberParameter<T> createOrgUnitMemberParameter(final String uid, final Set<T> members,
			final Class<T> type, final PageableData pageableData)
	{
