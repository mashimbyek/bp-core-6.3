...
{
	@InjectMocks
	private DefaultCommerceCartMetadataUpdateStrategy defaultCommerceCartMetadataUpdateStrategy = new DefaultCommerceCartMetadataUpdateStrategy();
	...
	public void setup()
	{
		defaultCommerceCartMetadataUpdateStrategy.setCommerceCartMetadataUpdateMethodHooks(Arrays.asList(
				commerceCartMetadataUpdateMethodHook1, commerceCartMetadataUpdateMethodHook2));
	}
	...
	public void shouldHaveNoModification()
	{
		...
		given(metadataParameter.isRemoveExpirationTime()).willReturn(false);
		given(metadataParameter.isEnableHooks()).willReturn(true);
	...
	public void shouldModifyRemoveExpirationTime()
	{
		...
		given(metadataParameter.getDescription()).willReturn(Optional.empty());
		given(metadataParameter.getExpirationTime()).willReturn(Optional.empty());
		given(metadataParameter.isRemoveExpirationTime()).willReturn(true);
		given(metadataParameter.isEnableHooks()).willReturn(true);
