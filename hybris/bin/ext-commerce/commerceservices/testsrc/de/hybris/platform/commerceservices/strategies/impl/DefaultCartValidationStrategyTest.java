...
public class DefaultCartValidationStrategyTest
{
	...
	public void setUp()
	{
		...
		given(cart.getEntries()).willReturn(Collections.singletonList(cartEntry));
		given(cartEntry.getQuantity()).willReturn(20L);
	...
	{
        AbstractOrderEntryProductInfoModel infoModel = Mockito.mock(AbstractOrderEntryProductInfoModel.class);
		given(infoModel.getProductInfoStatus()).willReturn(ProductInfoStatus.ERROR);
		given(cartEntry.getProductInfos()).willReturn(Collections.singletonList(infoModel));

		given(productService.getProductForCode(cartEntry.getProduct().getCode())).willReturn(product);
		given(commerceStockService.getStockLevelForProductAndBaseStore(product, store)).willReturn(Long.valueOf(22));
		given(cartEntry.getQuantity()).willReturn(20L);

		final CommerceCartModification modification = strategy.validateCartEntry(cart, cartEntry);
		Assert.assertEquals(CommerceCartModificationStatus.CONFIGURATION_ERROR, modification.getStatusCode());
	}

	@Test
	public void testNoConfigurationErrors()
	{
        AbstractOrderEntryProductInfoModel infoModel = Mockito.mock(AbstractOrderEntryProductInfoModel.class);
		given(infoModel.getProductInfoStatus()).willReturn(ProductInfoStatus.SUCCESS);
		given(cartEntry.getProductInfos()).willReturn(Collections.singletonList(infoModel));

		given(productService.getProductForCode(cartEntry.getProduct().getCode())).willReturn(product);
		given(commerceStockService.getStockLevelForProductAndBaseStore(product, store)).willReturn(22L);
	...
	@Test
	public void testNotConfigurableProduct()
	{
		given(cartEntry.getProductInfos()).willReturn(Collections.emptyList());
