...
public class OracleInLimitationTest extends ServicelayerTransactionalTest
{
	...
	public void shouldNotCachePreviousStatement()
	{
		try
		{
			...
			// when
			final List<TitleModel> result = flexibleSearchService.<TitleModel> search(
					"SELECT {pk} FROM {Title} WHERE {name} in (?names)", ImmutableMap.of("names", names(1, 2, 5))).getResult();

			// then
			assertThat(result).hasSize(3);

			// and when
			final List<TitleModel> result2 = flexibleSearchService.<TitleModel> search(
					"SELECT {pk} FROM {Title} WHERE {name} in (?names)", ImmutableMap.of("names", names(1, 2))).getResult();
	...
	public void selectTitleByCodeAndName()
	{
		try
		{
			...
			// when
			final List<TitleModel> result = flexibleSearchService.<TitleModel> search(
					"SELECT {pk} FROM {Title} WHERE {name} in (?names) AND {code} in (?codes)",
					ImmutableMap.of("names", names(1, 2, 3, 4, 8, 10), "codes", codes(1, 3, 4, 10))).getResult();
	...
	public void shouldWorkIfMoreThan1000GroupsAreAssignedToPrincipal()
	{
		...
		final List<Object> result = flexibleSearchService.search(
				"SELECT  {pgr:source} FROM {PrincipalGroupRelation AS pgr} WHERE {pgr:target} in (?groups)",
				ImmutableMap.of("groups", userGroups)).getResult();

		final List<Object> distinctResult = flexibleSearchService.search(
				"SELECT DISTINCT {pgr:source} FROM {PrincipalGroupRelation AS pgr} WHERE {pgr:target} in (?groups)",
				ImmutableMap.of("groups", userGroups)).getResult();
	...
	public void shouldCorrectlyHandleJoinStatement()
	{
		...
		// when
		final List<Object> result = flexibleSearchService.search(
				"SELECT {o.PK} FROM {Order AS o JOIN User AS u ON {o.user}={u.PK}} WHERE {u.name} IN (?names)",
				ImmutableMap.of("names", names)).getResult();
	...
	private ProductModel createProduct(final ClassificationSystemVersionModel catalogVersion, final String classificationCode)
	{
		...
		final ClassificationClassModel classificationClass = flexibleSearchService
				.<ClassificationClassModel> searchUnique(new FlexibleSearchQuery("SELECT {PK} FROM {"
						+ ClassificationClassModel._TYPECODE + "} WHERE {code} = ?code", ImmutableMap.of("code", classificationCode)));
