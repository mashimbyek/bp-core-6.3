...
public class DefaultCatalogSynchronizationServiceTest extends BaseSynchronizationStatusServiceTest
{
	...
	@Test
	public void shouldSnychronizePartiallyWithSyncJobAndConfigSynchronous()
	{
		...
		ProductModel srcBeingRemoved = sourceProducts.get(5);
		ProductModel tgtToRemove = modelService.get(CatalogManager.getInstance()
		...
		Assertions.assertThat(srcBeingRemoved.getItemModelContext().isRemoved()).isTrue();
		ProductModel srcBeingChanged = sourceProducts.get(4);
		...
		Assertions.assertThat(tgtToRemove.getItemModelContext().isRemoved()).isTrue();
	}
	...
	public void shouldSynchronizeRemovedItems()
	{
		...
		syncService.synchronizeFully(source, target, NUMBER_OF_THREADS);
		CatalogVersionAssert.assertThat(source).hasNumOfProductsAndReferences(NUM_OF_PRODUCTS, NUM_OF_PRODUCTS * NUM_OF_REFS);
		CatalogVersionAssert.assertThat(target).hasNumOfProductsAndReferences(NUM_OF_PRODUCTS, NUM_OF_PRODUCTS * NUM_OF_REFS);

		final List<ProductModel> removed = removeSomeProductsInSource(sourceProducts);
		assertThat(removed).isNotEmpty();
		CatalogVersionAssert.assertThat(source).hasNumOfProductsAndReferences(NUM_OF_PRODUCTS - removed.size(),
				(NUM_OF_PRODUCTS - removed.size()) * NUM_OF_REFS - getAdditionalReferencesBeingDeleted(NUM_OF_REFS));

		syncService.synchronizeFully(source, target, NUMBER_OF_THREADS);

		CatalogVersionAssert.assertThat(target).hasNumOfProductsAndReferences(NUM_OF_PRODUCTS - removed.size(),
				(NUM_OF_PRODUCTS - removed.size()) * NUM_OF_REFS - getAdditionalReferencesBeingDeleted(NUM_OF_REFS));
	}
	...
	public void testSynchronizationForOneProduct()
	{
		...
		// given
		ProductModel product = createProductInCatalogVersion(String.format("%s%s", "test_product", RandomStringUtils.random(3)),
				source);
		modelService.save(product);
		SyncItemJobModel syncItemJob = createSyncJob(source, target);

		// when
		performSynchronization(syncItemJob, Lists.newArrayList(product), prepareSyncConfig());
		product = modelService.get(product.getPk());

		ProductModel productCounterpart = catalogSynchronizationService.getSynchronizationTargetFor(syncItemJob, product);
		...
		ProductModel sourceProductCounterpart = resolveCounterpart(target, sourceProduct, "code");
	...
	public void testPullSynchronizationForOneProduct()
	{
		...
		ProductModel targetProduct = createProductInCatalogVersion(
	...
	public void testGetApplicableItemsForDifferentSyncItemJobs()
	{
		...
		// then
		Assertions.assertThat(applicableForBaseSyncJob).isNotEmpty().containsExactly(product);
		Assertions.assertThat(applicableForAdditionalSyncJob).isNotEmpty().containsExactly(thumbnail);
	...
	public void testGetSyncCounterpartItems()
	{
		...
		ProductModel productCounterpart = resolveCounterpart(target, product, "code");
		...
		// then
		assertSyncInfoEquals(syncInfo, SyncItemStatus.IN_SYNC, syncItemJob.getPk(), product.getPk());
		Assertions.assertThat(targetItem).isNotNull();
		Assertions.assertThat(targetItem.getCode()).isEqualTo(product.getCode());
		Assertions.assertThat(targetItem.getCatalogVersion()).isEqualTo(target);

		Assertions.assertThat(sourceItem).isNull();
		...
		Assertions.assertThat(sourceItem).isNotNull();
		Assertions.assertThat(sourceItem.getCode()).isEqualTo(product.getCode());
		Assertions.assertThat(sourceItem.getCatalogVersion()).isEqualTo(source);

		Assertions.assertThat(targetItem).isNull();
	...
	private List<ProductModel> removeSomeProductsInSource(List<ProductModel> sourceProducts )
	{
	...
	private void modifyProductsInSource(List<ProductModel> sourceProducts)
	{
		int i = 0;
	...
	private List<ProductModel> createCatalogWithNProducts( final int NUM_OF_PRODUCTS, int NUM_OF_REFS )
	{
   	final List<ProductModel> sourceProducts = createProducts(NUM_OF_PRODUCTS, source);
