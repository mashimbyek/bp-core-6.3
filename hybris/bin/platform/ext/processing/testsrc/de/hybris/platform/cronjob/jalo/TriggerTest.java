...
public class TriggerTest extends HybrisJUnit4TransactionalTest
{
	...
	@Ignore("PLA-5870 PLA-11490")
	public void testActiveTriggers()
	{
		/*
		 * DAMN FUCK TESTS // triggers every second final Trigger trigger1 = manager.createTrigger(cronJob, 10, -1, -1,
		 * -1, -1, -1, null,true ); registerForRemoval(trigger1); trigger1.setActive(true); // current time final Calendar
		 * now = Utilities.getDefaultCalendar(); // to be sure that at least one hour is passed by now.roll(Calendar.HOUR,
		 * 1); assertNotNull("trigger is null", manager.getActiveTriggers(now.getTime().getTime())); assertEquals( 1,
		 * manager.getActiveTriggers(now.getTime().getTime()).size()); assertEquals( trigger1,
		 * manager.getActiveTriggers(now.getTime().getTime()).iterator().next()); CronJob cronJob2 =
		 * manager.createCronJob(batchJob,"cronJob2",true); registerForRemoval(cronJob2); final Trigger trigger2 =
		 * manager.createTrigger(cronJob2, 10, -1, -1, -1, -1, -1,null,true ); registerForRemoval(trigger2);
		 * trigger2.setActive(true); Collection ACTIVE_TRIGGERS = Arrays.asList( new Trigger[]{ trigger1, trigger2 } ); //
		 * to be sure that at least one hour is passed by now.roll(Calendar.HOUR, 1); assertNotNull("trigger is null",
		 * manager.getActiveTriggers(now.getTime().getTime())); assertEquals( 2,
		 * manager.getActiveTriggers(now.getTime().getTime()).size()); for (Iterator iter =
		 * manager.getActiveTriggers(now.getTime().getTime()).iterator(); iter.hasNext(); ) { final Trigger trigger =
		 * (Trigger)iter.next(); assertTrue( "Trigger " + trigger + " exists!", ACTIVE_TRIGGERS.contains( trigger ) ); }
		 */
	}
