...
public class ReturnOrderPageController extends AbstractSearchPageController
{
	...
	public String confirmCancelOrderPage(@PathVariable("orderCode") final String orderCode,
			@ModelAttribute("orderEntryReturnForm") final OrderEntryReturnForm orderEntryReturnForm,
			final Model model, final RedirectAttributes redirectModel) throws CMSItemNotFoundException
	{
		try
		{
			...
			breadcrumbs.add(new Breadcrumb(MY_ACCOUNT_ORDER + orderCode, getMessageSource()
					.getMessage("text.account.order.orderBreadcrumb", new Object[] { orderDetails.getCode() }, "Order {0}",
							getI18nService().getCurrentLocale()), null));
			breadcrumbs.add(new Breadcrumb(MY_ACCOUNT_ORDER + orderCode + "/return",
					getMessageSource().getMessage("text.account.returnOrder", null, getI18nService().getCurrentLocale()), null));
			breadcrumbs.add(new Breadcrumb("#",
	...
	public String submitCancelOrderPage(@PathVariable("orderCode") final String orderCode,
			@ModelAttribute("orderEntryCancelForm") final OrderEntryReturnForm orderEntryReturnForm,
			final Model model, final RedirectAttributes redirectModel) throws CMSItemNotFoundException
	{
		try
		{
			...
			GlobalMessages.addFlashMessage(redirectModel, GlobalMessages.INFO_MESSAGES_HOLDER, getMessageSource()
					.getMessage("text.account.return.success", null, getI18nService().getCurrentLocale()), null);
			return REDIRECT_TO_ORDERS_HISTORY_PAGE;
		}
