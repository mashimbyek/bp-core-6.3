...
/**
 * Default implementation of {@link AddOnConfigDataImportService}
 */
public class DefaultAddOnConfigDataImportService implements AddOnConfigDataImportService
{
	...
	protected boolean executeCMSImport(final String extensionName, final ImpexMacroParameterData macroParameters)
	{
		...
		imported |= getSetupImpexService().importImpexFile(path + "cms-content.impex", macroParameters, false, false);
		...
		{
			imported |= getSetupImpexService().importImpexFile(path + "cms-responsive-content.impex", macroParameters, false, false);
		}
