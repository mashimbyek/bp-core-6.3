...
ACC.address.bindCountrySpecificUnitAddressForms = function (){
		$('#unitAddressCountrySelector select').filter('#address\\.country').on("change", function (){
			var options = {
				'addressCode': '',
...
ACC.address.onUnitRegionChanged = function() {
	$("select#unitaddress\\.region").change(function (){
		...
		$.getJSON(url, function(data){
			var $cities = $('select#unitaddress\\.townCity'), defaultOption= $('select#unitaddress\\.townCity > option:first');
...
ACC.address.onUnitCityChanged = function() {
	$("select#unitaddress\\.townCity").change(function (){
		...
		if(!cityCode.isEmpty()) {
			...
			$.getJSON(url, function (data)	{
				var $districts = $('select#unitaddress\\.district'), defaultOption= $('select#unitaddress\\.district > option')[0];
