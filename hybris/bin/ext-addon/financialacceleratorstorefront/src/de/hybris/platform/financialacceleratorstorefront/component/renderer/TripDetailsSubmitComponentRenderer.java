		...
		extends AbstractFormSubmitComponentRenderer<C>
{
	...
	protected String calculateNumberOfDays(final PageContext pPageContext)
	{
		...
		final DateTime start = formatter.parseDateTime(
				(String) pPageContext.getSession().getAttribute(FinancialacceleratorstorefrontConstants.TRIP_DETAILS_START_DATE));
		final DateTime end = formatter.parseDateTime(
				(String) pPageContext.getSession().getAttribute(FinancialacceleratorstorefrontConstants.TRIP_DETAILS_END_DATE));
