 ...
 */
package de.hybris.platform.chinesetaxinvoiceaddon.setup;
...
/**
 * Import when hac using update
 */
@SystemSetup(extension = ChinesetaxinvoiceaddonConstants.EXTENSIONNAME)
public class ChineseTaxinvoiceAddonSystemSetup extends AbstractSystemSetup
{
	@SuppressWarnings("unused")
	private static final Logger LOG = Logger.getLogger(ChineseTaxinvoiceAddonSystemSetup.class);
		...
		for (final ImportData data : importData)
		{
			for (final String contentCatalogName : data.getContentCatalogNames())
			{
				getSetupImpexService()
						.importImpexFile(String.format("/%s/import/contentCatalogs/%sContentCatalog/cms-responsive-content.impex",
