...
<script type="text/javascript">
	...
	<c:choose>
		<c:when test="${pageType == 'PRODUCT'}">
			...
			<c:set var="categories" value="" />
				...
				tracker.setEcommerceView('${product.code}',  // (required) SKU: Product unique identifier
				...
				<c:when test="${searchPageData.pagination.totalNumberOfResults > 0}">
					<c:if test="${not empty breadcrumbs}">
						tracker.trackSiteSearch('${searchPageData.freeTextSearch}',  // Search keyword searched for
								...
								tracker.setCustomData('categoryName', '${categoryName}'));
					</c:if>
				</c:when>
				<c:otherwise>
						tracker.trackSiteSearch('${searchPageData.freeTextSearch}',  // Search keyword searched for
		...
		<c:when test="${pageType == 'ORDERCONFIRMATION'}">
			<c:forEach items="${orderData.entries}" var="entry">
				tracker.setCustomVariable(1,"ec_id","${orderData.code}","page");
				<c:forEach items="${entry.product.categories}" var="category">
					<c:set var="categories">${categories},'${category.code}'</c:set>
				</c:forEach>
				tracker.setEcommerceView('${entry.product.code}',  // (required) SKU: Product unique identifier
				...
				tracker.addEcommerceItem('${entry.product.code}',  // (required) SKU: Product unique identifier
						'${entry.product.name}', // (optional) Product name
						[${fn:substringAfter(categories, ',')}],  // (optional) Product category. You can also specify an array of up to 5 categories eg. ["Books", "New releases", "Biography"]
						'${entry.product.price.value}',  // (recommended) Product price
			...
			tracker.trackEcommerceOrder('${orderData.code}',  // (required) Unique Order ID
	...
	function trackContinueCheckoutClick_piwik() {
		tracker.setCustomVariable(1,"ec_id","${cartData.code}","page");
