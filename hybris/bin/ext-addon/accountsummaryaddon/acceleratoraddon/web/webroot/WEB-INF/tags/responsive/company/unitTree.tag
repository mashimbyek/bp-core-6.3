...
<spring:url htmlEscape="false" value="/my-company/organization-management/accountsummary-unit/details" var="unitDetailUrl">
    ...
    <div class="panel-heading">
        ...
        <c:choose>
            <c:when test="${fn:length(node.children) > 0}">
                <a data-toggle="collapse" data-parent="#accordion${loopIndex}" href="#collapse${loopIndex}" class="accordion-toggle ${collapsed}"></a>
            </c:when>
            <c:otherwise>
                <c:set var="noSubs" value="no-subs"/>
            </c:otherwise>
        </c:choose>
        <a href="${unitDetailUrl}"  class="accordion-lnk ${noSubs} ${node.active? '' : 'disabled'}">
        ${fn:escapeXml(node.name)}
        </a>
    </div>
