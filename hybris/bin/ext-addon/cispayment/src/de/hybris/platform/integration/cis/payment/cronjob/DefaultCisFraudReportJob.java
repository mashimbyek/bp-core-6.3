...
public class DefaultCisFraudReportJob extends AbstractJobPerformable<CisFraudReportCronJobModel>
{
	...
	public PerformResult perform(final CisFraudReportCronJobModel cronJob)
	{
		try
		{
			...
			final CisFraudReportResult response = getFraudClient().generateFraudReport(cronJob.getCode(), getTenantId(), request);

			LOG.info("===>>> continue...");
			if (response != null)
			{
				final List<CisFraudTransactionResult> transactionResults = response.getTransactions();
