...
public class CisCreateSubscriptionResultPopulator implements Populator<CisPaymentProfileResult, CreateSubscriptionResult>
{
	@Override
	public void populate(final CisPaymentProfileResult cisPaymentProfileResult, final CreateSubscriptionResult target)
			throws ConversionException
	{
		Assert.notNull(cisPaymentProfileResult, "Parameter source cannot be null.");
		Assert.notNull(target, "Parameter target cannot be null.");

		final Map<String, String> venderResponses = cisPaymentProfileResult.getVendorResponses().convertToMap();
		...
		subscriptionInfoData.setSubscriptionID(cisPaymentProfileResult.getHref());
		subscriptionInfoData.setSubscriptionSignedValue(cisPaymentProfileResult.getId());
		subscriptionInfoData
				.setSubscriptionIDPublicSignature(venderResponses.get("paySubscriptionCreateReply_subscriptionIDPublicSignature"));
		...
		if (cisPaymentProfileResult.getValidationResult() != null)
		{
			...
			{
				authReplyData
						.setCcAuthReplyReasonCode(Integer.valueOf(cisPaymentProfileResult.getValidationResult().getVendorReasonCode()));
			}
		}
		...
		if (customerAddress != null)
		{
			...
			customerInfoData
					.setBillToCountry(customerAddress.getCountry() == null ? null : customerAddress.getCountry().toUpperCase());
		}
