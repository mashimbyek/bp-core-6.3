...
<%@ taglib prefix="multi-checkout" tagdir="/WEB-INF/tags/addons/chinacheckoutaddon/desktop/checkout/multi" %>
				...
				<form:form id="silentOrderPostForm" name="silentOrderPostForm" commandName="sopPaymentDetailsForm" class="create_update_payment_form" action="${paymentFormUrl}" method="POST">
					...
					<div class="description"><spring:theme code="checkout.multi.paymentMethod.addPaymentDetails.enterYourCardDetails"/></div>
							...
							<c:if test="${not empty paymentInfos}">
								<button type="button" class="positive clear view-saved-payments" id="viewSavedPayments" >View Saved Payments</button>
							</c:if>
					...
					<c:forEach items="${paymentInfos}" var="paymentInfo" varStatus="status">
						<div class="paymentEntry">
							<form action="${request.contextPath}/checkout/multi/select-payment-method" method="GET">
								...
								<button type="submit" class="positive right" tabindex="${status.count + 21}">Use this payment info</button>
							</form>
							<form:form action="${request.contextPath}/checkout/multi/remove-payment-method" method="POST">
								<input type="hidden" name="paymentInfoId" value="${paymentInfo.id}"/>
								<button type="submit" class="negative remove-payment-item right" tabindex="${status.count + 22}">Remove</button>
							</form:form>
						</div>
					</c:forEach>
					</div>
