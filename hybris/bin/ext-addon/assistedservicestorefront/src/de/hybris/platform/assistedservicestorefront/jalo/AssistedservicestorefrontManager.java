 ...
 */
public class AssistedservicestorefrontManager extends GeneratedAssistedservicestorefrontManager
{
	/** Edit the local|project.properties to change logging behavior (properties 'log4j.*'). */
	private static final Logger LOG = Logger.getLogger(AssistedservicestorefrontManager.class.getName());
	 ...
	 */
	public static AssistedservicestorefrontManager getInstance()
	{
		return (AssistedservicestorefrontManager) Registry.getCurrentTenant().getJaloConnection().getExtensionManager().getExtension(
				AssistedservicestorefrontConstants.EXTENSIONNAME);
	}
	 ...
	 */
	public AssistedservicestorefrontManager() // NOPMD 
	{
		if (LOG.isDebugEnabled())
		{
			LOG.debug("constructor of AssistedservicestorefrontManager called.");
		}
	}
	...
	public void init()
	{
		if (LOG.isDebugEnabled())
		{
			LOG.debug("init() of AssistedservicestorefrontManager called. " + getTenant().getTenantID());
		}
	}
	...
	public void destroy()
	{
		if (LOG.isDebugEnabled())
		{
			LOG.debug("destroy() of AssistedservicestorefrontManager called, current tenant: " + getTenant().getTenantID());
		}
	}
