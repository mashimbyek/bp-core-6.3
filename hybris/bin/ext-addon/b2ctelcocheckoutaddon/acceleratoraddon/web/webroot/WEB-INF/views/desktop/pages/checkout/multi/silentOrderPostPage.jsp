...
<template:page pageTitle="${pageTitle}">
	...
	<c:if test="${not empty paymentFormUrl}">
		<div class="span-14 append-1">
			<div id="checkoutContentPanel" class="clearfix">
				...
				<form:form id="silentOrderPostForm" name="silentOrderPostForm" commandName="sopPaymentDetailsForm" class="create_update_payment_form" action="${paymentFormUrl}" method="POST">
					...
					<div class="cardForm">
                        ...
                        <div class="form-additionals" style="display: none;">
					...
					<div class="form-actions">
						<c:if test="${empty cancelUrl }">
							<c:url value="/checkout/multi/delivery-method/choose" var="qualifiedCancelUrl"/>
						</c:if>
						<c:if test="${not empty cancelUrl }">
							<c:url value="${cancelUrl}" var="qualifiedCancelUrl"/>
						</c:if>
						<a href="${qualifiedCancelUrl}"><spring:theme code="checkout.multi.cancel" text="Cancel"/></a>
						<button class="positive submit_silentOrderPostForm" tabindex="20">
			...
			<c:if test="${not empty checkoutSteps}">
