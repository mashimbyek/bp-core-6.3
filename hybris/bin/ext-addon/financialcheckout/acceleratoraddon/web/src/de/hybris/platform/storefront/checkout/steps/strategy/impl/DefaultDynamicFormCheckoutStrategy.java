...
public class DefaultDynamicFormCheckoutStrategy implements DynamicFormCheckoutStrategy
{
			...
			final String formPageId, final String progressBarId)
	{
		...
		{
			newStep.setStepVisitedAnalysisStrategyList(
					((InsuranceCheckoutStep) formCheckoutStepPlaceholder).getStepVisitedAnalysisStrategyList());
			newStep.setCheckoutStepValidatorList(
					((InsuranceCheckoutStep) formCheckoutStepPlaceholder).getCheckoutStepValidatorList());
			newStep.setStepEnabledAnalysisStrategyList(
					((InsuranceCheckoutStep) formCheckoutStepPlaceholder).getStepEnabledAnalysisStrategyList());
		}
			...
			final List<AbstractCheckoutStepController.CheckoutSteps> originalCheckoutSteps)
	{
		...
		for (final AbstractCheckoutStepController.CheckoutSteps checkoutStep : originalCheckoutSteps)
		{
			if (formCheckoutStepPlaceholder.getProgressBarId().equals(checkoutStep.getProgressBarId()))
			{
				...
				{
					combinedCheckoutSteps.add(new AbstractInsuranceCheckoutStepController.InsuranceCheckoutSteps(
							formCheckoutStep.getProgressBarId(), StringUtils.remove(formCheckoutStep.currentStep(), "redirect:"),
							progressKey, formCheckoutStep.getCurrentStatus(), formCheckoutStep.getIsEnabled()));
					progressKey++;
				}
			}
			else
			{
				if (checkoutStep instanceof AbstractInsuranceCheckoutStepController.InsuranceCheckoutSteps)
				{
					...
					combinedCheckoutSteps.add(new AbstractInsuranceCheckoutStepController.InsuranceCheckoutSteps(
							checkoutStep.getProgressBarId(), step.getUrl(), progressKey, step.getStatus(), step.getIsEnabled()));
				}
				else
				{
					combinedCheckoutSteps.add(new AbstractInsuranceCheckoutStepController.InsuranceCheckoutSteps(
							checkoutStep.getProgressBarId(), checkoutStep.getUrl(), progressKey));
				}
				progressKey++;
			}
		}
