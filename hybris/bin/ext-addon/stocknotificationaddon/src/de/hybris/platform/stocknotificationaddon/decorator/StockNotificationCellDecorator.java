 ...
 */
package de.hybris.platform.stocknotificationaddon.decorator;
 ...
 */
public class StockNotificationCellDecorator implements CSVCellDecorator
{

	private static final Logger logger = LoggerFactory.getLogger(StockNotificationCellDecorator.class);
	...
	public String decorate(final int position, final Map<Integer, String> impexLine)
	{
		...
		final String impexContent = impexLine.get(new Integer(position));
		final String[] actions = StringUtils.split(impexContent, ",");
		final String stockAlertAction = actions[0];

		final StringBuilder fql = new StringBuilder();
		final Map<String, Object> params = new HashMap<>();
		final Collection<CatalogVersionModel> catalogVersions = new ArrayList<>();

		final CatalogVersionModel catalogVersionModel = catalogVersionService.getCatalogVersion("electronicsContentCatalog",
				"Staged");
		...
		if (!CollectionUtils.isEmpty(result.getResult()))
		{
			...
			if(logger.isInfoEnabled()){
				logger.info(("Initialization: " + StockNotificationCellDecorator.class.getName() + " found actions are " + addToCartActions
						.toString()));
			}

			addToCartActions.remove("AddToCartAction");
			if (!addToCartActions.contains(stockAlertAction))
			{
				addToCartActions.add(0, stockAlertAction);
			}
